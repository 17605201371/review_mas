# Run comparison dashboard v1

- candidate: `mimo_v25_negqty_recoverycap_guard3_qhyg_targetneg_freeformrevneg_reviewissuebundle_hardneg20_mt7_b4w2_api2_r8t600_tok1536_20260627_162459.jsonl` (label: p28_hardneg20, papers: 20)
- dashboard_mode: `smoke`

## Protection lines

| metric | op | threshold | note | actual | pass |
|---|---|---|---|---|---|
| `final_nonreal_strong_support` | `==` | 0 |  | 0 | PASS |
| `low_score_promoted_strong` | `==` | 0 |  | 0 | PASS |
| `final_report_leakage_paper_count` | `==` | 0 |  | 0 | PASS |
| `synthetic_marker_in_supporting_count` | `==` | 0 |  | 0 | PASS |
| `negative_evidence_unlinked_to_flaw` | `==` | 0 |  | 0 | PASS |
| `semantic_negative_without_review_relation_count` | `==` | 0 |  | 0 | PASS |
| `positive_or_neutral_negative_candidate_count` | `==` | 0 |  | 0 | PASS |
| `recovery_safe_resolution_or_clean_state` | `>=` | 11 | smoke scaled from 20/39 | 18 | PASS |
| `hygiene_delta_or_safe_block_or_clean_state` | `>=` | 11 | smoke scaled from 20/39 | 16 | PASS |
| `real_strong_support_total` | `>=` | 16 | smoke scaled from 30/39 | 51 | PASS |
| `independent_support_group_total` | `>=` | 13 | smoke scaled from 24/39 | 51 | PASS |
| `empirical_real_strong_support_count` | `>=` | 11 | smoke scaled from 20/39 | 41 | PASS |
| `claims_with_deep_support` | `>=` | 5 | smoke scaled from 8/39 | 29 | PASS |
| `final_support_missing_verified_quote_count` | `==` | 0 |  | 0 | PASS |
| `support_trace_overridden_by_negative_burden_count` | `==` | 0 |  | 0 | PASS |
| `evidence_formation_dead_loop_count` | `==` | 0 |  | 0 | PASS |
| `programmatic_specific_locator_count` | `>=` | 10 | smoke scaled from 18/39 | 38 | PASS |

**Overall protection: PASS**

## Evidence formation health

| metric | p28_hardneg20 |
|---|---|
| `evidence_agent_worker_turns` | 73 |
| `evidence_json_status_turns` | 65 |
| `evidence_json_valid_turns` | 65 |
| `evidence_json_partial_recovered_turns` | 0 |
| `evidence_json_fallback_turns` | 0 |
| `evidence_json_fallback_rate_pct` | 0 |
| `evidence_json_no_json_object_turns` | 0 |
| `evidence_json_invalid_json_turns` | 0 |
| `evidence_json_truncated_turns` | 0 |
| `evidence_json_prompt_chars_median` | 7826 |
| `evidence_json_raw_chars_median` | 1304 |
| `quote_bank_nonzero_turns` | 73 |
| `payload_evidence_item_total` | 115 |
| `evidence_agent_nonempty_payload_turns` | 54 |
| `evidence_agent_question_only_turns` | 2 |
| `first_support_fallback_turns` | 0 |
| `model_adapter_quote_first_rewrite_count` | 0 |
| `model_adapter_strength_downgrade_count` | 0 |
| `small_model_quote_bank_augmentation_count` | 46 |
| `evidence_formation_dead_loop_count` | 0 |

## Positive support

| metric | p28_hardneg20 |
|---|---|
| `real_strong_support_total` | 51 |
| `independent_support_group_total` | 51 |
| `diagnostic_independent_support_group_total` | 63 |
| `claims_with_2plus_independent_or_diagnostic_support` | 19 |
| `empirical_real_strong_support_count` | 41 |
| `method_real_strong_support_count` | 10 |
| `table_or_figure_real_strong_support_count` | 28 |
| `result_or_experiment_real_strong_support_count` | 12 |
| `ablation_real_strong_support_count` | 1 |
| `abstract_real_strong_support_count` | 0 |
| `verified_moderate_support_total` | 13 |
| `moderate_diagnostic_support_total` | 13 |
| `moderate_absorbed_into_final_strong_count` | 26 |
| `moderate_remaining_diagnostic_count` | 13 |
| `diagnostic_support_signal_total` | 64 |
| `papers_with_real_strong_support` | 15 |
| `papers_with_empirical_support` | 13 |
| `papers_with_deep_support` | 13 |
| `positive_coverage_gap_papers` | 5 |
| `empirical_coverage_gap_papers` | 7 |
| `deep_support_gap_papers` | 7 |
| `claims_with_real_strong_support` | 35 |
| `claims_with_empirical_real_strong_support` | 28 |
| `claims_with_deep_support` | 29 |
| `claims_with_2plus_independent_support` | 14 |
| `primary_claim_total` | 60 |
| `primary_claims_with_real_strong_support` | 33 |
| `primary_claims_with_empirical_support` | 26 |
| `primary_claims_with_deep_support` | 27 |
| `zero_real_papers` | 5 |
| `final_support_total` | 51 |
| `final_support_direct_strong_count` | 25 |
| `final_support_promoted_from_medium_count` | 26 |
| `final_support_semantic_weak_promotion_count` | 0 |
| `near_miss_deep_moderate_support_count` | 2 |
| `near_miss_method_moderate_support_count` | 0 |
| `near_miss_specific_locator_moderate_count` | 2 |
| `near_miss_promoted_to_final_count` | 1 |
| `support_trace_total` | 107 |
| `support_trace_included_count` | 51 |
| `support_trace_dropped_count` | 56 |
| `support_trace_hygiene_filtered_count` | 12 |
| `support_trace_overridden_by_negative_burden_count` | 0 |
| `support_trace_weak_support_depth_count` | 24 |
| `support_trace_semantic_mismatch_count` | 15 |
| `support_trace_duplicate_quote_count` | 3 |
| `support_trace_missing_verified_quote_count` | 1 |
| `final_support_specific_locator_count` | 38 |
| `final_support_weak_locator_count` | 13 |

## Negative & flaws

| metric | p28_hardneg20 |
|---|---|
| `negative_evidence_candidate_count` | 2 |
| `negative_evidence_candidate_raw_count` | 2 |
| `review_negative_verified_count` | 0 |
| `reviewer_absence_verified_count` | 2 |
| `reviewer_absence_verified_claim_count` | 17 |
| `reviewer_absence_verified_flaw_count` | 2 |
| `total_review_negative_verified_count` | 2 |
| `quote_grounded_review_issue_count` | 0 |
| `obligation_grounded_review_issue_count` | 2 |
| `obligation_grounded_review_issue_claim_count` | 2 |
| `reviewer_candidate_review_issue_count` | 0 |
| `reviewer_candidate_review_issue_claim_count` | 0 |
| `claim_obligation_review_issue_count` | 2 |
| `claim_obligation_review_issue_claim_count` | 2 |
| `verified_review_issue_count` | 2 |
| `verified_review_issue_claim_count` | 2 |
| `review_issue_bundle_count` | 2 |
| `verified_issue_without_recovery_count` | 0 |
| `review_issue_candidate_total` | 4 |
| `review_issue_candidate_verified` | 0 |
| `review_issue_candidate_retrieval_gap_rejected` | 0 |
| `review_issue_candidate_generic_item_rejected` | 0 |
| `review_issue_candidate_counterevidence_rejected` | 0 |
| `review_issue_candidate_missing_inventory_rejected` | 4 |
| `review_issue_candidate_off_claim_rejected` | 0 |
| `review_issue_type_missing_ablation` | 0 |
| `review_issue_type_missing_baseline` | 1 |
| `review_issue_type_unfair_or_weak_baseline` | 0 |
| `review_issue_type_insufficient_evaluation` | 0 |
| `review_issue_type_missing_robustness_or_generalization` | 0 |
| `review_issue_type_evaluation_protocol_risk` | 0 |
| `review_issue_type_efficiency_cost_gap` | 1 |
| `review_issue_type_scope_overclaim` | 0 |
| `review_issue_type_result_claim_mismatch` | 0 |
| `review_issue_type_method_support_gap` | 0 |
| `review_issue_type_reproducibility_gap` | 0 |
| `paper_text_negative_candidate_count` | 27 |
| `author_limitation_only_count` | 0 |
| `prior_work_limitation_count` | 0 |
| `positive_or_neutral_negative_candidate_count` | 0 |
| `resource_or_scope_context_negative_candidate_count` | 0 |
| `semantic_negative_without_review_relation_count` | 0 |
| `semantic_negative_rejected_by_review_relation_count` | 3 |
| `scope_limitation_as_verified_negative_count` | 0 |
| `quote_bank_salvage_generated_negative_count` | 0 |
| `negative_evidence_linked_to_flaw_count` | 2 |
| `negative_evidence_linked_to_flaw_raw_count` | 2 |
| `negative_evidence_unlinked_to_flaw` | 0 |
| `verified_negative_flaw_count` | 3 |
| `verified_actionable_negative_flaw_count` | 3 |
| `verified_limitation_negative_flaw_count` | 0 |
| `negative_type_direct_contradiction` | 0 |
| `negative_type_negative_result` | 0 |
| `negative_type_missing_ablation` | 0 |
| `negative_type_missing_baseline` | 1 |
| `negative_type_unfair_or_weak_baseline` | 0 |
| `negative_type_insufficient_evaluation` | 0 |
| `negative_type_missing_robustness_or_generalization` | 0 |
| `negative_type_evaluation_protocol_risk` | 0 |
| `negative_type_efficiency_cost_gap` | 2 |
| `negative_type_reproducibility_gap` | 0 |
| `negative_type_scope_overclaim` | 0 |
| `negative_type_result_claim_mismatch` | 0 |
| `negative_type_scope_limitation` | 0 |
| `synced_actionable_negative_type_count` | 0 |
| `negative_type_neutral_control_context` | 0 |
| `negative_type_generic_gap` | 0 |
| `verified_potential_concern_count` | 3 |
| `grounded_weakness_count` | 0 |
| `assessment_limitation_flaw_count` | 43 |
| `negative_grounding_conflict_count` | 33 |
| `invalid_negative_evidence_id_count_legacy` | 33 |
| `negative_semantic_anchor_conflict_count` | 33 |
| `generic_gap_semantic_rejected_count` | 0 |
| `negative_evidence_semantic_rejected_count` | 3 |
| `downgraded_flaw_count` | 3 |
| `potential_concern_count` | 3 |
| `diagnosis_pending_potential_concern_count` | 117 |
| `diagnosis_pending_potential_concern_claim_count` | 61 |
| `diagnosis_pending_concern_recorded_count` | 3 |
| `diagnosis_pending_concern_recorded_claim_count` | 3 |
| `coverage_gap_potential_concern_count` | 29 |
| `reviewer_inferred_potential_concern_count` | 29 |
| `final_potential_concern_total` | 15 |
| `diagnosis_pending_type_missing_ablation` | 10 |
| `diagnosis_pending_type_missing_baseline` | 22 |
| `diagnosis_pending_type_unfair_or_weak_baseline` | 0 |
| `diagnosis_pending_type_insufficient_evaluation` | 30 |
| `diagnosis_pending_type_missing_robustness_or_generalization` | 0 |
| `diagnosis_pending_type_evaluation_protocol_risk` | 6 |
| `diagnosis_pending_type_efficiency_cost_gap` | 5 |
| `diagnosis_pending_type_reproducibility_gap` | 13 |
| `diagnosis_pending_type_scope_overclaim` | 12 |
| `diagnosis_pending_type_result_claim_mismatch` | 0 |
| `diagnosis_pending_type_method_support_gap` | 19 |

## Coverage gaps (deterministic · primary-claim · unsupported)

| metric | p28_hardneg20 |
|---|---|
| `verified_coverage_gap_count` | 29 |
| `coverage_gap_potential_concern_count` | 29 |
| `reviewer_inferred_potential_concern_count` | 29 |
| `final_potential_concern_total` | 15 |
| `primary_claims_with_requirement_gaps` | 49 |

## State contamination

| metric | p28_hardneg20 |
|---|---|
| `state_contamination_count` | 38 |
| `state_contamination_count_legacy` | 38 |
| `harmful_state_contamination_count` | 0 |
| `repairable_state_warning_count` | 0 |
| `conservative_state_warning_count` | 38 |
| `state_hygiene_warning_count` | 38 |
| `weak_target_warning_count` | 38 |
| `repairable_contamination_target_count` | 0 |
| `conservative_contamination_target_count` | 38 |
| `blocked_fallback_contamination_target_count` | 0 |
| `blocked_empty_contamination_target_count` | 0 |
| `contamination_unsupported_with_strong_support` | 0 |
| `contamination_zero_real_support` | 5 |
| `contamination_stale_gap_persistence` | 0 |
| `contamination_unsupported_flaw_escalation` | 0 |
| `contamination_negative_evidence_overclaim` | 0 |
| `contamination_evidence_misbinding` | 33 |
| `contamination_meta_leakage` | 0 |
| `contamination_stale_flaw_persistence` | 0 |
| `contamination_harmful_recovery_risk` | 0 |
| `target_gate_real_target` | 0 |
| `target_gate_weak_target` | 38 |
| `target_gate_fallback_target` | 0 |
| `target_gate_empty_target` | 0 |

## Contested support

| metric | p28_hardneg20 |
|---|---|
| `contested_support_total` | 4 |
| `contested_final_support_total` | 2 |
| `claims_with_contested_support` | 2 |
| `claims_with_contested_final_support` | 2 |
| `open_conflict_count` | 40 |
| `contested_relation_final_count` | 5 |
| `contested_relation_added_count` | 6 |
| `contested_relation_effective_count` | 4 |
| `conflict_to_contested_resolution_count` | 0 |
| `negative_verified_target_preserved_count` | 9 |
| `diagnosis_pending_concern_commit_count` | 3 |
| `diagnosis_pending_concern_added_count` | 3 |
| `mark_contested_commit_count` | 6 |
| `mark_contested_with_positive_support_count` | 5 |
| `mark_contested_with_verified_negative_evidence_count` | 6 |
| `mark_contested_final_view_count` | 6 |
| `contested_relation_with_positive_support_count` | 4 |
| `contested_relation_with_verified_negative_evidence_count` | 5 |
| `contested_relation_final_view_count` | 5 |

## Gap cleanup & locator

| metric | p28_hardneg20 |
|---|---|
| `evidence_gap_open_count` | 25 |
| `evidence_gap_resolved_count` | 46 |
| `evidence_gap_superseded_count` | 0 |
| `evidence_gap_not_assessable_count` | 2 |
| `state_hygiene_open_gap_count` | 24 |
| `state_hygiene_stale_gap_count` | 1 |
| `targetless_open_gap_count` | 0 |
| `meta_or_context_open_gap_count` | 0 |
| `actionable_targeted_open_gap_count` | 2 |
| `diagnostic_targeted_open_gap_count` | 23 |
| `targeted_open_gap_count` | 25 |
| `assessment_limitation_open_gap_count` | 0 |
| `unresolved_open_count` | 60 |
| `unresolved_open_raw_count` | 157 |
| `unresolved_resolved_count` | 0 |
| `unresolved_deferred_count` | 155 |
| `targetless_unresolved_deferred_count` | 0 |
| `programmatic_specific_locator_count` | 38 |
| `programmatic_weak_locator_count` | 13 |
| `programmatic_locator_type_table_count` | 14 |
| `programmatic_locator_type_figure_count` | 11 |
| `programmatic_locator_type_section_count` | 12 |
| `programmatic_locator_type_algorithm_count` | 0 |
| `programmatic_locator_type_theorem_count` | 1 |
| `programmatic_locator_type_generic_count` | 13 |
| `programmatic_high_confidence_locator_count` | 38 |
| `programmatic_low_confidence_locator_count` | 0 |

## Recovery

| metric | p28_hardneg20 |
|---|---|
| `recovery_attempted` | 25 |
| `recovery_patch_validated` | 15 |
| `recovery_patch_committed` | 12 |
| `recovery_committed` | 12 |
| `recovery_success` | 12 |
| `hygiene_delta_improved` | 6 |
| `diagnosis_pending_recorded_layer` | 3 |
| `recovery_effective_repair` | 6 |
| `recovery_no_effect_commit` | 0 |
| `recovery_harmful_commit_risk` | 0 |
| `recovery_safe_resolution` | 23 |
| `recovery_safe_resolution_or_clean_state` | 18 |
| `hygiene_delta_or_safe_block` | 17 |
| `hygiene_delta_or_safe_block_or_clean_state` | 16 |
| `recovery_safe_blocked_weak_target` | 2 |
| `recovery_safe_blocked_terminal_target` | 9 |
| `recovery_terminal_turns` | 9 |
| `recovery_repeat_allowed_false_turns` | 9 |
| `recovery_target_gate_real_target_turns` | 9 |
| `recovery_target_gate_negative_verified_target_turns` | 9 |
| `recovery_target_gate_diagnosis_pending_target_turns` | 3 |
| `recovery_target_gate_weak_target_turns` | 4 |
| `recovery_target_gate_fallback_target_turns` | 0 |
| `recovery_target_gate_empty_target_turns` | 0 |
| `recovery_patch_operation_reject_patch_turns` | 13 |
| `recovery_patch_operation_downgrade_final_to_candidate_turns` | 0 |
| `recovery_patch_operation_route_to_assessment_limitation_turns` | 3 |
| `recovery_patch_operation_downgrade_claim_to_unsupported_turns` | 0 |
| `recovery_patch_operation_mark_contested_turns` | 6 |
| `recovery_patch_operation_record_diagnosis_pending_concern_turns` | 3 |
| `recovery_patch_operation_resolve_stale_gap_turns` | 0 |

## Recovery case audit

| metric | p28_hardneg20 |
|---|---|
| `recovery_case_rows` | 27 |
| `recovery_case_audit_error_count` | 0 |
| `recovery_case_decision_hygiene_error_count` | 0 |
| `recovery_case_verified_review_negative_repair` | 0 |
| `recovery_case_verified_review_issue_repair` | 3 |
| `recovery_case_reviewer_inferred_negative_repair` | 0 |
| `recovery_case_verified_negative_flaw_lifecycle_downgrade` | 0 |
| `recovery_case_verified_review_issue_lifecycle_downgrade` | 0 |
| `recovery_case_reviewer_inferred_flaw_lifecycle_downgrade` | 0 |
| `recovery_case_state_hygiene_repair` | 0 |
| `recovery_case_assessment_limitation_routing` | 0 |
| `recovery_case_effective_repair_without_verified_negative` | 3 |
| `recovery_case_flaw_lifecycle_downgrade_needs_manual_review` | 0 |
| `recovery_case_effective_repair_needs_manual_review` | 0 |
| `recovery_case_attempted_not_committed` | 15 |
| `recovery_case_committed_not_effective` | 6 |
| `recovery_case_effective_repair_turns` | 6 |
| `recovery_case_effective_repair_not_verified_negative_repair` | 6 |
| `recovery_case_turns_with_verified_review_negative_evidence` | 0 |
| `recovery_case_turns_with_verified_review_issue_bundle_evidence` | 3 |
| `recovery_case_turns_with_reviewer_absence_audit_evidence` | 0 |
| `recovery_case_evidence_bucket_verified_review_negative` | 0 |
| `recovery_case_evidence_bucket_obligation_grounded_review_issue` | 3 |
| `recovery_case_evidence_bucket_reviewer_absence_audit` | 0 |
| `recovery_case_evidence_bucket_author_limitation_only` | 0 |
| `recovery_case_evidence_bucket_prior_work_limitation` | 0 |
| `recovery_case_evidence_bucket_positive_or_neutral_support` | 0 |
| `recovery_case_evidence_bucket_resource_or_scope_context` | 0 |
| `recovery_case_evidence_bucket_untrusted_model_output` | 0 |
| `recovery_case_evidence_bucket_quote-bank-negative-grounding_candidate` | 2 |
| `recovery_case_evidence_bucket_fallback-extraction_candidate` | 0 |
| `recovery_case_evidence_bucket_system_recovery_salvage_candidate` | 0 |
| `recovery_case_evidence_bucket_support_only` | 0 |
| `recovery_case_evidence_bucket_not_verified_or_unknown` | 1 |
| `recovery_case_evidence_bucket_missing_evidence_id` | 3 |

## Hygiene

| metric | p28_hardneg20 |
|---|---|
| `final_nonreal_strong_support` | 0 |
| `low_score_promoted_strong` | 0 |
| `final_report_leakage_paper_count` | 0 |
| `user_report_leakage_paper_count` | 0 |
| `synthetic_marker_in_supporting_count` | 0 |
| `negative_evidence_unlinked_to_flaw` | 0 |

## Recovery failure codes

| code | p28_hardneg20 | interpreted safety outcome |
|---|---|---|
| `ACTIONABLE_CONCERN_PRESERVED` | 1 | **safe_terminal_block (verified potential concern preserved)** |
| `BLOCKED_BY_POLICY` | 9 | **safe_blocked_patch (policy restriction/abstention)** |
| `EVIDENCE_SEMANTIC_MISMATCH` | 1 | **safe_blocked_patch (semantic evidence validation mismatch)** |
| `EVIDENCE_TARGET_MISMATCH` | 1 | **safe_blocked_patch (missing or unverified IDs)** |
| `INVALID_STATUS_TRANSITION` | 1 | **unclassified_failure_requires_review** |
| `SUCCESS` | 12 | **recovery_patch_committed** |

## Final decision distribution

| decision | p28_hardneg20 |
|---|---|
| `reject` | 20 |
