...
from agent_system.review_prompts import MANAGER_PROMPT


@AgentRegistry.register("Review Manager Agent")
class ReviewManagerAgent(BaseReviewAgent):
    def __init__(self, wg_id: str, tokenizer: PreTrainedTokenizer, processor, config: Any):
        super().__init__("Review Manager Agent", MANAGER_PROMPT, wg_id=wg_id, tokenizer=tokenizer, processor=processor, config=config)
