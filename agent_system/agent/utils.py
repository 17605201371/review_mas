...
def _has_valid_think_tag(original_str: str) -> bool:
    think_start_count = original_str.count("<think>")
    think_end_count = original_str.count("</think>")
    think_start_idx = original_str.find("<think>")
    think_end_idx = original_str.find("</think>")
    return (
        think_start_count == 1 and
        think_end_count == 1 and
...
def json_projection(
    ...
    start_tag: str = "<json>",
    end_tag: str = "</json>",
    check_think_tag: bool = True,
) -> Tuple[List[str], List[Any], np.ndarray]:
    """Extract tagged JSON payloads and validate them."""
    normalized_texts = [""] * len(text_repsonses)
    payloads: List[Any] = [None] * len(text_repsonses)
    valids = [0] * len(text_repsonses)

    for i, original_str in enumerate(text_repsonses):
        ...
        if check_think_tag and not _has_valid_think_tag(original_str):
            continue

        json_str = original_str[start_idx + len(start_tag):end_idx].strip()
        json_str = _strip_json_fence(json_str)
        try:
        ...
        payloads[i] = payload
        normalized_texts[i] = json.dumps(payload, ensure_ascii=False, sort_keys=True)
        valids[i] = 1

    return normalized_texts, payloads, np.array(valids, dtype=bool)
