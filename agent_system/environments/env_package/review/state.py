...
_NEGATIVE_EVIDENCE_TYPE_ALIASES = {
    ...
    "robustness_gap": "missing_robustness_or_generalization",
    "generalization_gap": "missing_robustness_or_generalization",
    "generalisation_gap": "missing_robustness_or_generalization",
    "missing_robustness": "missing_robustness_or_generalization",
    "missing_generalization": "missing_robustness_or_generalization",
    "missing_generalisation": "missing_robustness_or_generalization",
    "limited_evaluation": "insufficient_evaluation",
    "weak_evaluation": "insufficient_evaluation",
    "narrow_evaluation": "insufficient_evaluation",
    "missing_evaluation": "insufficient_evaluation",
    "insufficient_baseline": "missing_baseline",
    "missing_comparison": "missing_baseline",
    "weak_baseline": "unfair_or_weak_baseline",
    "unfair_baseline": "unfair_or_weak_baseline",
    "weak_or_unfair_baseline": "unfair_or_weak_baseline",
    "no_ablation": "missing_ablation",
    "missing_component_ablation": "missing_ablation",
    "missing_component_analysis": "missing_ablation",
    "runtime_gap": "efficiency_cost_gap",
    "compute_cost_gap": "efficiency_cost_gap",
    "cost_gap": "efficiency_cost_gap",
}
...
_MODEL_CLAIMED_EVIDENCE_VERIFICATION_FIELDS = frozenset(
    {
        ...
        "verified_source_span_start",
        "verified_source_span_end",
        "verified_quote_match_type",
        "verified_locator_quality",
        "verified_source_bucket",
        "verified_claim_overlap_score",
        "semantic_grounding_label",
        "semantic_grounding_reasons",
        "semantic_alignment_score",
        "semantic_grounding_checked",
        "quote_evidence_semantic_mismatch",
        "quote_bank_canonicalized",
        "quote_bank_claim_overlap_fallback_used",
        "quote_bank_claim_overlap_fallback_quote_id",
        "quote_bank_claim_overlap_fallback_source_bucket",
        "quote_bank_claim_overlap_fallback_score",
        "semantic_weak_promotion_used",
        "semantic_weak_promotion_reason",
        "strength_promotion_from_medium_used",
        "strength_promotion_reason",
        "review_negative_checked",
        "review_negative_label",
        "review_negative_reason",
        "review_negative_relation_score",
    }
)
...
_SEMANTIC_NEGATIVE_TERMS_RE = re.compile(
    r"\b(contradict|contradicts|contradicted|refute|refutes|weakens|weaken|undermine|undermines|unsupported|does not support|not supported|(?:do|does|did)\s+not\s+(?:prove|provide|show|report|evaluate|compare|include|establish)|not\s+(?:proven|proved|provided|reported|evaluated|compared|included|established)|lack|lacks|lacked|lacking|absent|insufficient|without|missing|open question|no significant|not significant|(?:minimal|negligible|limited|marginal|trivial|insignificant)\s+(?:performance\s+|accuracy\s+)?(?:gain|gains|improvement|improvements|benefit|benefits)|yields?\s+(?:minimal|little|marginal|negligible|limited)\s+(?:performance|accuracy|gain|gains|improvement|benefit)|less\s+favo[u]?rable|worse|underperform|fail|fails|failed|failure|limitation|limitations|threats? to validity|future work|not evaluated|not compared|no ablation|no baseline|missing baseline|no comparison|no evaluation)\b",
    re.IGNORECASE,
)
...
_COVERAGE_GENERIC_MISSING_ITEM_RE = re.compile(
    r"^\s*(?:all\s+)?(?:(?:relevant|recent|key|strong|standard|competitive|comprehensive|"
    r"additional|more|multiple|diverse|state[- ]of[- ]the[- ]art|sota)\s+)*"
...
def _paper_has_ablation_counterevidence_for_missing_claim(
    ...
    missing_text = " ".join(str(item or "") for item in missing_items if isinstance(item, (str, int, float))).strip()
    target_text = missing_text or str(claim_text or "")
    target_tokens = {
        token
        for token in _absence_target_tokens(target_text)
        if token not in {"ablation", "variant", "variants", "component", "module", "loss", "objective", "mechanism"}
    }
    ...
    for match in _ABLATION_COUNTEREVIDENCE_RE.finditer(paper_text):
        ...
        end = min(len(paper_text), match.end() + 1200)
        window = paper_text[start:end]
        if _ablation_counterevidence_window_resolves_target(window, target_tokens):
            return True
    return False
...
_QUOTE_GROUNDABLE_INTERNAL_NEGATIVE_CUE_RE = re.compile(
    r"\b("
    r"cannot\s+(?:be\s+)?(?:used\s+to\s+)?compar\w*"
    r"|not\s+comparable"
    r"|incomparable"
    r"|unfair\s+compar\w*"
    r"|worse"
    r"|lower\s+than"
    r"|underperform"
    r"|less\s+favo(?:u)?rable"
    r"|reduc(?:e|es|ed|ing)\s+(?:the\s+)?performance"
    r"|performance\s+(?:declin|degrad)"
    r"|degrad(?:e|es|ed|ation)"
    r"|minimal\s+performance\s+gains?"
    r"|no\s+significant\s+improv\w*"
    r"|fail(?:s|ed)?\s+to"
    r"|invalid\s+compar\w*"
    r"|memory\s+bottleneck"
    r"|latency"
    r"|runtime"
    r"|overhead"
    r"|trade[- ]?off"
...
def _assess_review_negative_relation(state: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
    """Decide whether a grounded negative-looking quote is a reviewer flaw.

    Paper quote existence is a grounding fact.  This gate is the separate review
    relation: the quote must weaken a target claim or ground a concrete reviewer
    concern.  Author self-limitations, prior-work limitations, and positive
    "we address limitations" text are not verified negative evidence.
    """
    ...
    if _SEMANTIC_ABSENCE_EXCERPT_LIMIT_RE.search(
        ...
        return _review_negative_assessment(
            ...
            "absence_claim_limited_to_quote_or_excerpt",
            relation_score,
        )

    if not (_has_review_negative_semantic_intent(evidence) or neg_type in NEGATIVE_EVIDENCE_TYPES_ALL):
        return _review_negative_assessment("not_negative_evidence", "record_has_no_negative_intent", relation_score)
    if str(evidence.get("verified_grounding_label") or "") not in VERIFIED_PAPER_GROUNDED_LABELS:
        return _review_negative_assessment("insufficient_paper_grounding", "quote_not_paper_grounded", relation_score)
    semantic_label = str(evidence.get("semantic_grounding_label") or "")
    if semantic_label == "semantic_author_limitation":
        reason = (
            f"{neg_type}_is_author_self_limitation"
            if neg_type
            else "author_scope_or_future_work_note"
        )
        return _review_negative_assessment("author_limitation_only", reason, relation_score)
    if semantic_label != "semantic_negative_verified":
        return _review_negative_assessment("insufficient_semantic_negative", "semantic_negative_verification_missing", relation_score)
    if neg_type in {"neutral_control_context", "generic_gap", "bibliographic_or_title_noise", "neutral_instruction_noise"}:
        return _review_negative_assessment("insufficient_claim_relation", f"negative_type_{neg_type}_not_review_negative", relation_score)
    if neg_type == "missing_ablation" and _paper_has_ablation_counterevidence_for_missing_claim(
        state or {},
        evidence,
        claim_text,
    ):
        return _review_negative_assessment(
            "insufficient_claim_relation",
            "existing_ablation_counterevidence_in_paper",
            relation_score,
        )

    quote_l = quote.lower()
    self_anchor = bool(_INTRO_NEGATIVE_SELF_ANCHOR_RE.search(quote))
    author_limitation = bool(_REVIEW_NEGATIVE_AUTHOR_LIMITATION_RE.search(quote_l))
    ...
    prior_work_negative_context = _negative_anchor_is_prior_work_context(quote)
    ...
    comparison_invalidation = bool(_comparison_invalidation_is_current_paper_issue(quote)) and (
        neg_type in {
            "missing_baseline",
            "unfair_or_weak_baseline",
            "insufficient_evaluation",
        ...
        or bool(
            re.search(
                r"\b(compare|comparison|cross[- ]method|across\s+methods?|between\s+methods?|"
                r"baseline|baselines|sota|state[- ]of[- ]the[- ]art)\b",
                " ".join([claim_text, evidence_text, quote]),
                re.IGNORECASE,
            )
        )
    )
    ...
    if neg_type in TRUE_PAPER_NEGATIVE_EVIDENCE_TYPES or neg_type in {"scope_overclaim", "scope_limitation"}:
        ...
        if contradicts_paper_claim:
            reason = (
                ...
                else "comparison_invalidation_weakens_claim"
                if comparison_invalidation
...
def _apply_review_negative_relation_label(state: Dict[str, Any], evidence: Dict[str, Any]) -> None:
    if not isinstance(evidence, dict):
        return
    if not (_has_review_negative_semantic_intent(evidence) or str(evidence.get("negative_evidence_type") or "").strip()):
        return
    evidence.update(_assess_review_negative_relation(state or {}, evidence))


def _review_negative_label_for_record(item: Dict[str, Any], state: Dict[str, Any]) -> str:
    if not isinstance(item, dict):
        return ""
    ...
    if existing_label:
        ...
        if existing_label == REVIEW_NEGATIVE_VERIFIED_LABEL:
            ...
            semantic_reasons = {
                str(reason)
                for reason in item.get("semantic_grounding_reasons") or []
            }
            high_risk_absence_label = bool(
                _negative_anchor_is_prior_work_context(quote)
                or str(item.get("review_negative_reason") or "") == "table_scope_absence_weakens_claim"
                or "table_scope_absence_verified" in semantic_reasons
            )
            if high_risk_absence_label:
                # Do not let stale high-risk runtime labels bypass the current
                # relation gate. This lets dashboard re-audits remove false
                # positives after guard tightening while preserving direct
                # contradiction / negative-result records.
                reassessed = str(_assess_review_negative_relation(state or {}, item).get("review_negative_label") or "").strip()
                if reassessed:
                    return reassessed
...
def _is_review_negative_verified_evidence_record(item: Dict[str, Any], state: Dict[str, Any]) -> bool:
    return _review_negative_label_for_record(item, state) == REVIEW_NEGATIVE_VERIFIED_LABEL
...
def _final_strong_guard(evidence: Dict[str, Any]) -> None:
    ...
    if _evidence_negative_locator_or_bucket_signal(evidence):
        ...
        if score < FINAL_STRONG_MIN_SCORE:
            ...
            semantic_weak_promotion = bool(evidence.get("semantic_weak_promotion_used"))
            locator_type = str(evidence.get("locator_type") or evidence.get("source_locator_type") or "").strip().lower()
            if not locator_type:
                locator_type = _locator_type_from_anchor(str(evidence.get("source_locator") or ""))
            anchor_exception = (
                not semantic_weak_promotion
                and
                decision_bucket in _TABLE_OR_RESULT_ANCHOR_BUCKETS
                and locator_type in {"table", "figure", "algorithm", "theorem"}
...
_UNTRUSTED_VERIFIER_SOURCE_LABELS = {
    "fallback",
    "fallback-extraction",
    "model-output",
    "model output",
    "model_generated",
    "manager_model",
    "system recovery salvage",
    "system_meta",
    "system-meta",
    "quote-bank-negative-grounding",
}


def _has_trusted_existing_grounding(evidence: Dict[str, Any]) -> bool:
    source = str((evidence or {}).get("source") or "").strip().lower()
    source_locator = str((evidence or {}).get("source_locator") or "").strip().lower()
    if source in _UNTRUSTED_VERIFIER_SOURCE_LABELS or source_locator in _UNTRUSTED_VERIFIER_SOURCE_LABELS:
        return False
    if str(evidence.get("verified_grounding_label") or "") not in VERIFIED_PAPER_GROUNDED_LABELS:
        return False
    ...
    return match_type in {
        ...
        "full_paper_exact_substring",
        "full_paper_normalized_substring",
    }
...
def _assess_quote_semantic_grounding(state: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    comparison_invalidation = bool(_comparison_invalidation_is_current_paper_issue(quote))
    quote_has_negative_anchor = bool(_SEMANTIC_NEGATIVE_TERMS_RE.search(quote) or comparison_invalidation)
    excerpt_limited_absence = bool(
        _SEMANTIC_ABSENCE_EXCERPT_LIMIT_RE.search(" ".join([evidence_text, agent_raw_quote]))
    )
    table_scope_absence_negative = _is_table_scope_absence_negative(
        state,
        evidence,
    ...
    if _evidence_has_negative_intent(evidence) and author_limitation_quote:
        ...
        if prior_work_negative_context:
            ...
            reasons.append("prior_work_negative_context")
        elif excerpt_limited_absence:
            label = "semantic_mismatch"
            reasons.append("absence_claim_limited_to_quote_or_excerpt")
        elif not quote_has_negative_anchor and not table_scope_absence_negative:
            reasons.append("quote_lacks_negative_anchor")
        if prior_work_negative_context:
            pass
        elif excerpt_limited_absence:
            pass
        elif table_scope_absence_negative:
...
def _verify_evidence_grounding_against_state(state: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _has_trusted_existing_grounding(item):
        _apply_comparison_invalidation_negative_override(state, item)
        _apply_review_negative_relation_label(state, item)
    ...
    raw_quote_text = str(item.get("raw_quote") or "")
    if not quote_bank and not (raw_quote_text.strip() and str((state or {}).get("paper_text") or "")):
        return item
    ...
    if matched_quote_id_entry is not None and (not raw_quote_text.strip() or not _evidence_has_negative_intent(item)):
        # For positive support, preserve the legacy quote_id canonicalization
        # path: the model may provide a paraphrase plus a program-extracted
        # quote_id anchor.  Negative/missing evidence is stricter below because
        # a mismatched quote_id can otherwise turn a reviewer concern into a
    ...
    elif matched_quote_id_entry is not None:
        canonical_quote = str(matched_quote_id_entry.get("raw_quote") or "")
        matched_entry_is_candidate_window = _is_candidate_window_quote_bank_entry(matched_quote_id_entry)
        quote_id_verification = _verify_quote_against_reference(
            raw_quote_text,
            canonical_quote,
            _normalize_int(matched_quote_id_entry.get("source_span_start"), default=-1),
        )
        prefix_verification: Optional[Dict[str, Any]] = None
        if quote_id_verification.get("verified_grounding_label") not in VERIFIED_PAPER_GROUNDED_LABELS:
            prefix_verification = _verify_quote_bank_prefix_canonicalization(
                raw_quote_text,
                canonical_quote,
                _normalize_int(matched_quote_id_entry.get("source_span_start"), default=-1),
            )
        if (
            quote_id_verification.get("verified_grounding_label") in VERIFIED_PAPER_GROUNDED_LABELS
            or (
                prefix_verification is not None
                and prefix_verification.get("verified_grounding_label") in VERIFIED_PAPER_GROUNDED_LABELS
            )
        ):
            _apply_quote_bank_entry_metadata(
                item,
                matched_quote_id_entry,
                canonicalize_quote=bool(canonical_quote) and not matched_entry_is_candidate_window,
                overwrite_locator=True,
            )
            if matched_entry_is_candidate_window:
                verification = quote_id_verification
                match_type = str(verification.get("verified_quote_match_type") or "quote_bank_raw_substring")
                verification["verified_quote_match_type"] = f"candidate_window_{match_type}"
                verification["verified_grounding_reason"] = (
                    "raw_quote matched a candidate-relevant paper window generated from the full paper text; "
                    "raw_quote was kept verbatim rather than canonicalized to the whole window"
                )
            elif prefix_verification is not None:
                ...
                verification["verified_quote_match_type"] = "quote_bank_id_canonical"
                verification["verified_grounding_reason"] = "raw_quote matched the supplied quote_id's program-extracted quote-bank item; raw_quote was canonicalized to that item"
        else:
            item["quote_id_mismatch_ignored"] = True
            item["quote_id_mismatch_ignored_quote_id"] = quote_id
            item["quote_id_mismatch_grounding_label"] = str(quote_id_verification.get("verified_grounding_label") or "")
            item["quote_id_mismatch_reason"] = "raw_quote did not match the supplied quote_id's quote-bank item; searching quote bank by raw_quote instead"
    ...
    if verification.get("verified_grounding_label") not in VERIFIED_PAPER_GROUNDED_LABELS:
        for entry in ordered_refs:
            ...
            if verification.get("verified_grounding_label") in VERIFIED_PAPER_GROUNDED_LABELS:
                canonical_quote = str(entry.get("raw_quote") or "")
                entry_is_candidate_window = _is_candidate_window_quote_bank_entry(entry)
                _apply_quote_bank_entry_metadata(
                    item,
                    entry,
                    canonicalize_quote=bool(canonical_quote) and not entry_is_candidate_window,
                    overwrite_locator=False,
                )
                if entry_is_candidate_window:
                    match_type = str(verification.get("verified_quote_match_type") or "quote_bank_raw_substring")
                    verification["verified_quote_match_type"] = f"candidate_window_{match_type}"
                    verification["verified_grounding_reason"] = (
                        "raw_quote matched a candidate-relevant paper window generated from the full paper text; "
    ...
    if (
        verification.get("verified_grounding_label") not in VERIFIED_PAPER_GROUNDED_LABELS
        and (_evidence_has_negative_intent(item) or str(item.get("negative_evidence_type") or "").strip())
    ):
        full_paper_verification = _verify_quote_against_full_paper(raw_quote_text, state or {})
        if full_paper_verification is not None:
            verification = full_paper_verification
...
def _verify_evidence_items_for_state(state: Dict[str, Any], evidence_items: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in evidence_items:
        if (
            ...
            and (
                str(item.get("source") or "").strip().lower() == ABSENCE_AUDIT_SOURCE
                or bool(item.get("absence_audit_verified"))
            )
        ):
            # Reviewer absence audit rows are deterministic coverage findings,
            # not paper quotes.  They are revalidated by
            # _is_reviewer_absence_audit_evidence_record; sending them through
            # the quote-grounding verifier rewrites them as
            # insufficient_paper_grounding and loses the audit provenance.
            verified.append(copy.deepcopy(item))
            continue
        verified.append(_verify_evidence_grounding_against_state(state, item))
    return verified
...
def _is_usable_support_grounding(evidence: Dict[str, Any]) -> bool:
    ...
    if (
        ...
        if evidence.get("runtime_evidence_verification_required") or evidence.get("model_claimed_verification_stripped"):
            return False
...
def _is_real_paper_claim_id_for_negative(claim_id: Any, declared_kind: Any = "") -> bool:
    """Claim gate for negative-evidence binding.

    Negative evidence must bind to an ordinary real paper claim, never to a
    context/fallback/recovery scaffold.  Raw-salvaged fallback claims can still
    be used by the diagnosis/search target gate, but they no longer count as
    final verified reviewer-negative evidence.
    """
    value = str(claim_id or "").strip().lower()
    if value.startswith(
        (
        ...
        return False
    return _is_real_paper_claim_id(claim_id, declared_kind)
...
def _real_claim_ids_from_state(state: Dict[str, Any]) -> set[str]:
    ...
    for item in state.get("claims", []) or []:
        ...
        if (
            isinstance(item, dict)
            and _is_real_paper_claim_id(claim_id, item.get("claim_kind"))
            and not _claim_gap_should_be_not_assessable(item)
        ):
...
def _normalize_flaw_item(item: Any, fallback_index: int) -> Optional[Dict[str, Any]]:
    ...
    weakness_type = _normalize_text(item.get("weakness_type") or item.get("flaw_type"), max_length=80)
    if weakness_type:
        normalized["weakness_type"] = weakness_type
    required_evidence_type = _normalize_text(item.get("required_evidence_type"), max_length=80)
    if required_evidence_type:
        normalized["required_evidence_type"] = required_evidence_type
    criterion = _normalize_text(item.get("criterion") or item.get("review_criterion"), max_length=80)
    if criterion:
        normalized["criterion"] = criterion
    source_stage = _normalize_text(item.get("source_stage"), max_length=80)
    if source_stage:
        normalized["source_stage"] = source_stage
    paper_side_rationale = _normalize_text(item.get("paper_side_rationale"), max_length=360)
    if paper_side_rationale:
        normalized["paper_side_rationale"] = paper_side_rationale
...
def _claim_gap_should_be_not_assessable(claim: Dict[str, Any]) -> bool:
    ...
    canonicalized_raw_salvage = bool(claim.get("paper_claim_canonicalized_from_raw_salvage"))
    origin = " ".join(
    ...
    return (
        ...
        or (
            origin_kind in {"context_synthesized", "raw_salvaged_claim_agent_output"}
            and not canonicalized_raw_salvage
        )
        or "context_derived" in origin
        or ("raw_salvage" in origin and not canonicalized_raw_salvage)
        or ("malformed_claim_agent_output" in origin and not canonicalized_raw_salvage)
    )
...
def _decision_real_claim_ids(state: Dict[str, Any]) -> set[str]:
    return {
        ...
        if (
            ...
            and _is_real_paper_claim_id(item.get("claim_id"), item.get("claim_kind"))
            and not _claim_gap_should_be_not_assessable(item)
        )
    }
...
_CLAIM_REQ_ROBUSTNESS_RE = re.compile(
    r"\b(robust(?:ness)?|generaliz(?:e|es|ed|ation)|generalisation|"
    r"unseen|out-of-domain|ood|cross-domain|external|corrupt(?:ed|ion)|"
    r"stress test|transfer(?:able|ability)|real-world)\b",
    re.IGNORECASE,
)
...
_CLAIM_REQ_EVALUATION_PROTOCOL_RE = re.compile(
    r"\b(protocol|setup|setting|settings|split|splits|train(?:ing)?/?test|"
    r"validation|held[- ]out|cross[- ]validation|seed|seeds|threshold|"
    r"hyperparameter|implementation setting|experimental setting|evaluation setting)\b",
    re.IGNORECASE,
)
...
_SUPPORT_BASELINE_RE = re.compile(
    ...
    r"against|outperform|surpass|better than|competitive|"
    r"benchmark|benchmarks|versus|prior (?:work|method|methods|approach|approaches)|"
    r"previous (?:work|method|methods|approach|approaches)|"
    r"existing (?:work|method|methods|approach|approaches))\b",
    re.IGNORECASE,
)
_SUPPORT_ABLATION_RE = re.compile(
    r"\b(ablation|ablat(?:e|ed|ion|ions)|component analysis|module analysis|"
    r"without (?:the )?\w+ module|remove(?:d|s)? (?:the )?\w+|"
    r"with and without|w/o\b|"
    r"different \w+ (?:operation|operations|configuration|configurations|variant|variants|setting|settings|component|components)|"
    r"(?:trained|evaluated|tested) with different)\b",
    re.IGNORECASE,
)
_CLAIM_SELF_REPORTS_ABLATION_RESULT_RE = re.compile(
    r"\b(?:ablation\s+(?:stud(?:y|ies)|analysis|experiment|experiments)|"
    r"ablat(?:ion|ed|e|es))\b.{0,80}\b(?:show|shows|demonstrate|demonstrates|"
    r"demonstrated|verify|verifies|verified|validate|validates|validated|importance|"
    r"contribution|effect|lead|leads|led|drop|drops|decrease|decreases|performance)\b|"
    r"\b(?:show|shows|shown|demonstrate|demonstrates|demonstrated|verify|verifies|"
    r"verified|validate|validates|validated)\b"
    r".{0,80}\bablation\b",
    re.IGNORECASE,
)
_SUPPORT_SCOPE_RE = re.compile(
    r"\b(across|multiple|several|diverse|various|unseen|out-of-domain|ood|"
    r"generaliz(?:e|es|ed|ation)|robust(?:ness)?|stress test|cross-domain|external)\b",
    re.IGNORECASE,
)
...
_SUPPORT_EFFICIENCY_RE = re.compile(
    r"\b(runtime|latency|memory|parameters?|flops|compute|computational cost|"
    r"wall-clock|throughput|speedup|efficiency|efficient|scalability|scalable)\b",
    re.IGNORECASE,
)
_SUPPORT_EVALUATION_PROTOCOL_RE = re.compile(
    r"\b(protocol|setup|setting|settings|split|splits|train(?:ing)?/?test|"
    r"validation|held[- ]out|cross[- ]validation|seed|seeds|hardware|"
    r"implementation setting|experimental setting|evaluation setting)\b",
    re.IGNORECASE,
)
...
_REQUIREMENT_TO_NEGATIVE_TYPE = {
    ...
    "robustness_or_generalization": "missing_robustness_or_generalization",
    "scope_coverage": "scope_overclaim",
    "evaluation_protocol": "evaluation_protocol_risk",
    "reproducibility_detail": "reproducibility_gap",
    "efficiency_cost": "efficiency_cost_gap",
    "method_detail": "method_support_gap",
}

# Review issue bundles cannot use a one-to-one requirement -> negative-type
# mapping. Real reviewer issues often express the same missing obligation as a
# mismatch/protocol/method flaw rather than the default "missing X" label.
_REQUIREMENT_TO_COMPATIBLE_REVIEW_ISSUE_TYPES = {
    "empirical_result": frozenset({
        "insufficient_evaluation",
        "result_claim_mismatch",
        "evaluation_protocol_risk",
        "negative_result",
    }),
    "baseline_or_comparison": frozenset({
        "missing_baseline",
        "unfair_or_weak_baseline",
        "result_claim_mismatch",
        "insufficient_evaluation",
    }),
    "ablation_or_component": frozenset({
        "missing_ablation",
        "method_support_gap",
        "result_claim_mismatch",
    }),
    "robustness_or_generalization": frozenset({
        "missing_robustness_or_generalization",
        "insufficient_evaluation",
        "scope_overclaim",
    }),
    "scope_coverage": frozenset({
        "scope_overclaim",
        "insufficient_evaluation",
        "missing_robustness_or_generalization",
    }),
    "evaluation_protocol": frozenset({
        "evaluation_protocol_risk",
        "insufficient_evaluation",
        "result_claim_mismatch",
    }),
    "efficiency_cost": frozenset({
        "efficiency_cost_gap",
        "insufficient_evaluation",
    }),
    "method_detail": frozenset({
        "method_support_gap",
        "missing_ablation",
        "result_claim_mismatch",
    }),
    "reproducibility_detail": frozenset({
        "reproducibility_gap",
        "method_support_gap",
    }),
}

_NEGATIVE_TYPE_TO_PREFERRED_REQUIREMENT = {
    "direct_contradiction": "empirical_result",
    "negative_result": "empirical_result",
    "missing_ablation": "ablation_or_component",
    "missing_baseline": "baseline_or_comparison",
    "unfair_or_weak_baseline": "baseline_or_comparison",
    "insufficient_evaluation": "empirical_result",
    "missing_robustness_or_generalization": "robustness_or_generalization",
    "evaluation_protocol_risk": "evaluation_protocol",
    "efficiency_cost_gap": "efficiency_cost",
    "method_support_gap": "method_detail",
    "scope_overclaim": "scope_coverage",
    "result_claim_mismatch": "empirical_result",
    "reproducibility_gap": "reproducibility_detail",
}

# Route 3 (deterministic): paper-level coverage gaps that can be rendered even
# without a reviewer candidate. Method/protocol/reproducibility gaps stay out of
# this global path because they need a candidate-specific inventory anchor.
...
_CANDIDATE_REVIEW_ISSUE_ACTIONABLE_REQUIREMENTS = frozenset({
    *_COVERAGE_GAP_ACTIONABLE_REQUIREMENTS,
    "evaluation_protocol",
    "method_detail",
...
_REQUIRED_EVIDENCE_TYPE_ALIASES = {
    "baseline": "baseline_or_comparison",
    "baselines": "baseline_or_comparison",
    "comparison": "baseline_or_comparison",
    "baseline_comparison": "baseline_or_comparison",
    "ablation": "ablation_or_component",
    "component": "ablation_or_component",
    "component_analysis": "ablation_or_component",
    "empirical": "empirical_result",
    "evaluation": "empirical_result",
    "result": "empirical_result",
    "results": "empirical_result",
    "scope": "scope_coverage",
    "generalization": "robustness_or_generalization",
    "generalisation": "robustness_or_generalization",
    "robustness": "robustness_or_generalization",
    "protocol": "evaluation_protocol",
    "evaluation_protocol": "evaluation_protocol",
    "evaluation_setup": "evaluation_protocol",
    "experimental_protocol": "evaluation_protocol",
    "efficiency": "efficiency_cost",
    "runtime": "efficiency_cost",
    "cost": "efficiency_cost",
    "reproducibility": "reproducibility_detail",
    "implementation": "reproducibility_detail",
...
_PAPER_INVENTORY_REQUIREMENT_PATTERNS: List[Tuple[str, re.Pattern[str]]] = [
    (
        ...
        re.compile(
            ...
            r"variant|variants|without (?:the )?[A-Za-z0-9_-]+|remove(?:d|s)? (?:the )?[A-Za-z0-9_-]+)\b",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        "robustness_or_generalization",
        re.compile(
            r"\b(robust|robustness|generalization|generalisation|out[- ]of[- ]domain|ood|"
            r"cross[- ]domain|transfer|stress test|sensitivity|unseen|external dataset)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "scope_coverage",
        re.compile(
            r"\b(dataset|datasets|benchmark|benchmarks|task|tasks|domain|domains|setting|settings|"
            r"scenario|scenarios|coverage|scope)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "evaluation_protocol",
        re.compile(
            r"\b(protocol|setup|setting|settings|split|splits|train|training set|validation|test set|"
            r"seed|seeds|threshold|metric|metrics|hyperparameter tuning|evaluation budget)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "efficiency_cost",
        re.compile(
            r"\b(runtime|latency|throughput|speedup|wall[- ]clock|memory|gpu|cpu|hardware|flops|"
            r"parameters?|compute|computational cost|computational time|efficiency|efficient)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "method_detail",
        re.compile(
            r"\b(method|algorithm|architecture|framework|pipeline|module|component|encoder|decoder|"
            r"objective|loss|optimizer|training procedure|implementation|gating|routing|network|layer)\b",
            re.IGNORECASE,
        ),
    ),
    (
        "reproducibility_detail",
        re.compile(
            r"\b(implementation details?|hyperparameters?|data split|training details?|optimizer|learning rate|"
            r"batch size|epochs?|code|reproducib(?:ility|le)|random seed|compute setup)\b",
            re.IGNORECASE,
        ),
    ),
]
...
def _paper_inventory_requirement_types(quote: str) -> List[str]:
    ...
    if "efficiency_cost" in requirements and not re.search(
        r"\b(runtime|latency|throughput|speedup|wall[- ]clock|memory|gpu|hardware|flops|parameters?|compute|computational cost|computational time|efficiency)\b",
        lower,
        re.IGNORECASE,
    ):
...
def _paper_inventory_candidate_spans(body: str) -> List[Tuple[int, str]]:
    spans: List[Tuple[int, str]] = []
    if not body:
        return spans
    for pattern in (_PAPER_INVENTORY_TABLE_RE, _PAPER_INVENTORY_SECTION_RE):
        for match in pattern.finditer(body):
            quote = _quote_fragment_around(body, match.start(), max_chars=300)
            if quote:
...
def _paper_inventory_from_text(state: Dict[str, Any], *, max_items: int = 20) -> Dict[str, Any]:
    ...
    for pos, quote in sorted(_paper_inventory_candidate_spans(body), key=lambda pair: pair[0]):
        ...
        if not requirements:
            continue
        verification = _verify_quote_against_reference(quote, body, 0)
        if verification.get("verified_grounding_label") not in VERIFIED_PAPER_GROUNDED_LABELS:
            continue
        seen_quotes.add(key)
...
def _review_issue_inventory_item_matches_requirement(
    ...
    if neg_type == "missing_ablation":
        return bool(requirements & {"ablation_or_component", "method_detail", "empirical_result"})
    ...
    if neg_type == "evaluation_protocol_risk":
        return bool(requirements & {"evaluation_protocol", "empirical_result"})
    if neg_type == "efficiency_cost_gap":
        return bool(requirements & {"efficiency_cost", "empirical_result", "baseline_or_comparison"})
...
def _review_issue_missing_items_look_truncated(missing_items: Sequence[str]) -> bool:
    ...
    for item in missing_items or []:
        ...
        if len(text) >= 60 and tokens and len(re.sub(r"[^A-Za-z0-9]+", "", tokens[-1])) <= 3:
            return True
    return False
...
def _review_issue_bundle_has_ablation_counterevidence(bundle: Dict[str, Any]) -> bool:
    ...
    claim_tokens = _semantic_tokens(claim_text)
    if _CLAIM_SELF_REPORTS_ABLATION_RESULT_RE.search(claim_text) and target_tokens & claim_tokens:
        return True
    inventory_text = " ".join(
        " ".join(
            [
    ...
    if not _REVIEW_ISSUE_ABLATION_RESOLUTION_RE.search(inventory_text):
        return False
    inventory_tokens = _semantic_tokens(inventory_text)
    return bool(target_tokens & inventory_tokens)
...
def _review_issue_observed_inventory_relevant_for_type(bundle: Dict[str, Any], neg_type: str) -> bool:
    ...
    if neg_type == "missing_ablation":
        return bool(
            re.search(
                r"\b(method|module|component|architecture|objective|loss|stage|block|head|"
                r"encoder|decoder|attention|ablation|variant|framework|figure|fig\.?)\b",
                inventory_text,
            )
        )
    if neg_type in {"insufficient_evaluation", "missing_robustness_or_generalization", "scope_overclaim"}:
    ...
    if neg_type == "evaluation_protocol_risk":
        return bool(
            re.search(
                r"\b(protocol|setup|setting|settings|split|validation|test|train|threshold|"
                r"seed|metric|hardware|budget|evaluation|experiment)\b",
                inventory_text,
            )
        )
    if neg_type == "efficiency_cost_gap":
        return bool(
            re.search(
                r"\b(runtime|latency|memory|flops|compute|computational\s+cost|computational\s+time|"
                r"running\s+time|wall[- ]clock|throughput|"
                r"efficiency|efficient|cost|hardware|gpu|parameter|parameters|table)\b",
                inventory_text,
            )
        )
    if neg_type in {"method_support_gap", "reproducibility_gap"}:
...
def _review_issue_related_work_only_window(window: str) -> bool:
    ...
    if not _REVIEW_ISSUE_FULL_TEXT_RELATED_WORK_RE.search(text[:260]):
        return False
    return not _REVIEW_ISSUE_FULL_TEXT_EVAL_RE.search(text)
    ...
    for raw_item in missing_items or []:
        ...
        for term in candidate_terms:
            ...
            if not clean or clean in _BASELINE_COUNTEREVIDENCE_GENERIC_TERMS:
                continue
            if len(clean) < 4 and not any(ch.isdigit() for ch in clean):
                continue
            terms.add(clean)
    return terms
...
def _ablation_counterevidence_window_resolves_target(window: str, target_tokens: Sequence[str]) -> bool:
    if not window or not target_tokens:
        return False
    ablation_signal_re = re.compile(
        rf"(?:{_ABLATION_REMOVAL_RE.pattern}|\\b(?:variant|variants|component\\s+analysis|module\\s+analysis|with\\s+and\\s+without)\\b)",
        re.IGNORECASE,
    )
    for match in ablation_signal_re.finditer(str(window or "")):
        prefix = str(window or "")[:match.start()]
        suffix = str(window or "")[match.end():]
        prev_boundaries = [prefix.rfind(ch) for ch in ("\n", ".", ";")]
        next_boundaries = [
            idx for idx in (suffix.find(ch) for ch in ("\n", ".", ";"))
            if idx >= 0
        ]
        start = max(prev_boundaries) + 1 if max(prev_boundaries) >= 0 else max(0, match.start() - 80)
        end = match.end() + (min(next_boundaries) if next_boundaries else 220)
        end = min(len(window), max(end, match.end() + 80))
    ...
    *,
    ...
    if neg_type == "missing_ablation":
        ...
        for window in windows:
            if _ABLATION_COUNTEREVIDENCE_RE.search(window) and _ablation_counterevidence_window_resolves_target(window, target_tokens):
                return "full_text_ablation_counterevidence"
        return ""
    ...
    if neg_type in _COVERAGE_BASELINE_NEGATIVE_TYPES:
        ...
        for window in windows:
            ...
            if _REVIEW_ISSUE_FULL_TEXT_EVAL_RE.search(window) and re.search(
                ...
                window,
                re.I,
            ) and _window_contains_missing_baseline_marker(window, missing_baseline_terms):
                return "full_text_baseline_or_comparison_counterevidence"
        return ""
    ...
    *,
    ...
    for candidate in view.get("reviewer_negative_candidates", []) or []:
        ...
        neg_type = _canonical_negative_evidence_type(
            candidate.get("negative_type") or candidate.get("review_issue_type") or candidate.get("issue_type")
        )
        if neg_type not in ACTIONABLE_NEGATIVE_EVIDENCE_TYPES:
            continue
        if neg_type in NOISE_NEGATIVE_TYPES or neg_type in {"generic_gap", "neutral_control_context"}:
        ...
        key = (claim_id, neg_type, "|".join(selected_requirements))
        if key in seen:
            continue
        seen.add(key)
        gap = dict(audit)
...
_HARD_NEGATIVE_DIAGNOSTIC_CHECKS = {
    ...
    "scope_coverage": {
        ...
        "criterion": "significance_contribution",
        "check": "Does the tested scope match the breadth of the claim?",
        "true_flaw_condition": "The claim is broad/general, but paper-side evidence explicitly limits the evaluated setting or contradicts the claimed scope.",
    },
    "robustness_or_generalization": {
        "negative_evidence_type": "missing_robustness_or_generalization",
        "criterion": "empirical_adequacy",
        "check": "Does the paper test robustness, transfer, out-of-domain, or generalization claims in the relevant settings?",
        "true_flaw_condition": "The claim asserts robustness/generalization, but paper-side evidence shows no robustness, transfer, external, or out-of-domain evaluation.",
    },
    "reproducibility_detail": {
        "negative_evidence_type": "reproducibility_gap",
        "criterion": "clarity_reproducibility",
        "check": "Does the paper provide enough implementation/training/data detail to support reproducibility-sensitive claims?",
        "true_flaw_condition": "The paper-side evidence shows missing or insufficient implementation, data split, or training details for reproducing the claim.",
    },
    "efficiency_cost": {
        "negative_evidence_type": "efficiency_cost_gap",
        "criterion": "practicality_efficiency",
        "check": "Does the paper provide runtime, memory, parameter, latency, FLOP, or compute-cost evidence for efficiency/scalability claims?",
        "true_flaw_condition": "The claim asserts efficiency, scalability, or practicality, but paper-side evidence omits or contradicts runtime, memory, latency, or compute-cost support.",
    },
    "method_detail": {
        "negative_evidence_type": "method_support_gap",
        "criterion": "technical_soundness",
        "check": "Does the paper provide method details or assumptions sufficient for this method claim?",
        "true_flaw_condition": "The method claim is central, but the paper-side evidence shows an unsupported assumption, incomplete mechanism, or missing technical detail.",
    },
}
...
_TARGETED_NEGATIVE_SEARCH_QUESTIONS = {
    "empirical_result": {
        "search_question": "Does the paper provide concrete experiment, result, table, or metric evidence that is sufficient for this empirical claim?",
        "expected_quote_cues": ["experiment", "result", "accuracy", "performance", "Table", "Figure", "metric", "dataset"],
        "target_locator_hint": "Experiments / Results / Evaluation / Table or Figure",
    },
    "baseline_or_comparison": {
        "search_question": "Does the paper compare this claimed improvement against appropriate baselines or prior methods?",
        "expected_quote_cues": ["baseline", "compare", "state-of-the-art", "prior method", "outperform", "Table"],
        "target_locator_hint": "Experiments / Baselines / Results tables",
    },
    "ablation_or_component": {
        "search_question": "Does the paper isolate the claimed component, module, or contribution with an ablation or component analysis?",
        "expected_quote_cues": ["ablation", "component", "variant", "without", "module", "contribution", "analysis"],
        "target_locator_hint": "Ablation / Analysis / Experiments",
    },
    "scope_coverage": {
        "search_question": "Does the evaluated scope match the breadth of this claim, or does the paper explicitly limit the setting?",
        "expected_quote_cues": ["dataset", "domain", "generalize", "scope", "limitation", "only", "setting"],
        "target_locator_hint": "Experiments / Discussion / Limitations",
    },
    "robustness_or_generalization": {
        "search_question": "Does the paper test the robustness, transfer, out-of-domain, unseen, or real-world generalization claimed here?",
        "expected_quote_cues": ["robustness", "generalization", "unseen", "out-of-domain", "OOD", "external", "corruption", "stress"],
        "target_locator_hint": "Robustness / Generalization / Experiments / Limitations",
    },
    "reproducibility_detail": {
        "search_question": "Does the paper provide implementation, training, data split, or hyperparameter detail needed to reproduce this claim?",
        "expected_quote_cues": ["implementation", "hyperparameter", "training", "data split", "code", "reproduce", "details"],
        "target_locator_hint": "Method / Implementation Details / Appendix",
    },
    "efficiency_cost": {
        "search_question": "Does the paper report runtime, memory, parameters, latency, FLOPs, or compute cost for this efficiency/scalability claim?",
        "expected_quote_cues": ["runtime", "latency", "memory", "parameters", "FLOPs", "compute", "cost", "scalability"],
        "target_locator_hint": "Experiments / Efficiency / Runtime / Resource Analysis",
    },
    "method_detail": {
        "search_question": "Does the paper provide method details or assumptions sufficient to support this method claim?",
        "expected_quote_cues": ["method", "objective", "assumption", "algorithm", "architecture", "theorem", "proof"],
        "target_locator_hint": "Method / Approach / Theory",
    },
}
...
def _build_support_survival_trace(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for evidence in state.get("evidence_map", []) or []:
        if not isinstance(evidence, dict):
            continue
        current_stance = str(evidence.get("stance") or "").strip()
        if current_stance in _NEGATIVE_EVIDENCE_STANCES:
            continue
        stance = str(evidence.get("initial_stance") or current_stance or "").strip()
        if stance not in {"supports", "partially_supports"}:
            continue
        if str(evidence.get("strength") or "").strip().lower() == "missing":
            continue
        claim_id = str(evidence.get("claim_id") or "")
...
def _review_negative_dedup_signature(record: Dict[str, Any], state: Dict[str, Any]) -> Tuple[str, str, str, str]:
    ...
    Multiple agents may rediscover the same paper quote in different turns.  The
    paper narrative should count that as one reviewer-discovered negative, not
    as multiple independent defects.  The signature therefore keys on the target
    claim, negative type, relation label, and the verified paper anchor when one
    is available.
    """
    if not isinstance(record, dict):
        return ("", "", "", "")
...
def _report_safe_negative_locator(record: Dict[str, Any]) -> str:
    """Return a paper-facing locator for a verified negative quote."""
    if not isinstance(record, dict):
        return ""
    for key in ("source_locator", "source"):
        raw = _normalize_text(record.get(key), max_length=100)
        if not raw:
            continue
        if _GENERIC_LOCATOR_RE.search(raw):
            continue
        if _is_report_meta_leakage_text(raw):
            continue
        return raw
    locator_type = str(record.get("locator_type") or record.get("source_locator_type") or "").strip().lower()
    if locator_type in {"table", "figure", "section", "algorithm", "theorem"}:
        return locator_type.title()
    return ""
...
def _auto_bind_unlinked_negative_evidence_flaws(view: Dict[str, Any], negative_evidence_ids: set[str]) -> List[str]:
    ...
    def _auto_bind_target_claim(record: Dict[str, Any], fallback_claim_id: str) -> Tuple[str, str]:
        fallback_text = ""
        fallback_claim: Optional[Dict[str, Any]] = None
        for claim in view.get("claims", []) or []:
            if isinstance(claim, dict) and str(claim.get("claim_id") or "").strip() == fallback_claim_id:
                fallback_claim = claim
                fallback_text = _normalize_text(claim.get("claim") or claim.get("text"), max_length=140)
                break
        quote_and_evidence = " ".join(
            str(record.get(key) or "")
            for key in ("raw_quote", "evidence", "binding_rationale", "negative_evidence_type")
        )
        if not _comparison_invalidation_is_current_paper_issue(quote_and_evidence):
            return fallback_claim_id, fallback_text
        best: Tuple[float, str, str] = (-999.0, fallback_claim_id, fallback_text)
        evidence_tokens = _semantic_tokens(quote_and_evidence)
        for claim in view.get("claims", []) or []:
            if not isinstance(claim, dict):
                continue
            claim_id = str(claim.get("claim_id") or "").strip()
            if not claim_id or not _is_real_paper_claim_id(claim_id):
                continue
            claim_text_full = str(claim.get("claim") or claim.get("text") or "")
            claim_text = _normalize_text(claim_text_full, max_length=140)
            claim_type = str(claim.get("claim_type") or "").strip()
            tags = {str(tag).strip() for tag in claim.get("coverage_tags", []) or []}
            score = _semantic_alignment_score(claim_text_full, quote_and_evidence)
            if claim_type in {"empirical", "comparison"} or tags & {"empirical", "comparison"}:
                score += 0.45
            if re.search(
                r"\b(compare|comparison|comparative|baseline|baselines|effectiveness|evaluation|"
                r"results?|across\s+methods?|between\s+methods?)\b",
                claim_text_full,
                re.IGNORECASE,
            ):
                score += 0.35
            if str(claim.get("status") or "") in {"supported", "partially_supported"}:
                score += 0.10
            if claim_type == "limitation_or_boundary" or tags & {"limitation", "scope"}:
                score -= 0.20
            if claim_id == fallback_claim_id:
                score += 0.05
            if evidence_tokens & _semantic_tokens(claim_text_full):
                score += 0.05
            if score > best[0]:
                best = (score, claim_id, claim_text)
        if best[1]:
            return best[1], best[2]
        return fallback_claim_id, fallback_text

    linked_ids: set[str] = set()
    auto_bound_by_signature: Dict[Tuple[str, str, str, str], Dict[str, Any]] = {}
    for flaw in view.get("flaw_candidates", []) or []:
        if not isinstance(flaw, dict):
            continue
        valid_ids = _flaw_valid_negative_evidence_ids(flaw, view)
        linked_ids.update(valid_ids)
        for eid in valid_ids:
            record = next(
                (
                    item for item in view.get("evidence_map", []) or []
                    if isinstance(item, dict) and str(item.get("evidence_id") or "") == str(eid)
                ),
                None,
            )
            if isinstance(record, dict):
                auto_bound_by_signature.setdefault(_review_negative_dedup_signature(record, view), flaw)
    evidence_by_id = {
        str(item.get("evidence_id") or ""): item
        for item in view.get("evidence_map", []) or []
        if isinstance(item, dict) and str(item.get("evidence_id") or "")
    ...
    for evidence_id in sorted(negative_evidence_ids):
        ...
        if not claim_found:
            continue
        flaw_claim_id, claim_text = _auto_bind_target_claim(record, claim_id)
        signature = _review_negative_dedup_signature(record, view)
        existing_auto_flaw = auto_bound_by_signature.get(signature)
        if isinstance(existing_auto_flaw, dict):
            for key in ("evidence_ids", "negative_evidence_ids"):
                values = existing_auto_flaw.setdefault(key, [])
        ...
        flaw_id = _slugify("flaw-auto-negative-binding", f"{flaw_claim_id}:{evidence_id}", existing_flaw_count + len(added_ids) + 1)
        (view.setdefault("flaw_candidates", [])).append(
            {
                ...
                "status": "candidate",
                "related_claim_ids": [flaw_claim_id] if flaw_claim_id else [],
        ...
        auto_bound_by_signature[signature] = view["flaw_candidates"][-1]
...
def build_decision_hygiene_view(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    view["decision_hygiene"] = {
        ...
        "reviewer_absence_verified_count": reviewer_absence_verified_count,
        "reviewer_absence_verified_claim_count": len(reviewer_absence_claim_ids),
        "reviewer_absence_verified_flaw_count": len(reviewer_absence_audit.get("flaw_ids", []) or []),
        "reviewer_absence_verified_type_counts": reviewer_absence_audit.get("type_counts", {}),
        "total_review_negative_verified_count": (
            quote_grounded_review_negative_count + reviewer_absence_verified_count
        ),
        # P27: real review issues are broader than copied negative quotes.
        # Quote-grounded reviewer negatives remain strict, while obligation-grounded
        # issues are verified by claim anchor + support-inventory mismatch.
        "quote_grounded_review_issue_count": quote_grounded_review_negative_count,
        ...
        "reviewer_inferred_potential_concern_count": (
            ...
            + len(recorded_extra_claim_ids)
        ),
        "final_potential_concern_total": (
            flaw_layer_counts.get("potential_concern", 0)
            + len(coverage_gap_not_already_flaw_claim_ids)
            + len(recorded_extra_claim_ids)
        ),
        ...
        **support_quality,
        ...
        "negative_evidence_candidate_count": len(negative_evidence_signatures),
        "negative_evidence_candidate_raw_count": len(negative_evidence_ids),
        "review_negative_verified_count": quote_grounded_review_negative_count,
        ...
        "review_negative_false_positive_samples": review_negative_false_positive_samples,
        "negative_evidence_linked_to_flaw_count": len(linked_negative_evidence_signatures),
        "negative_evidence_linked_to_flaw_raw_count": len(linked_negative_evidence_ids),
        "negative_evidence_unlinked_to_flaw_count": max(0, len(negative_evidence_signatures) - len(linked_negative_evidence_signatures)),
        "negative_evidence_binding_retry_candidate_count": max(0, len(negative_evidence_signatures) - len(linked_negative_evidence_signatures)),
...
def _grounded_active_flaws_for_decision(state: Dict[str, Any], severities: set[str]) -> List[Dict[str, Any]]:
    ...
    for flaw in state.get("flaw_candidates", []) or []:
        ...
        if not flaw.get("evidence_ids"):
            continue
        if _classify_flaw_final_view_layer(flaw, state) != "grounded_weakness":
            continue
        flaws.append(flaw)
    return flaws
...
def infer_final_recommendation_view(state: Dict[str, Any], manager_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    active_major_like = [
        ...
        and _flaw_has_negative_grounding(flaw, decision_state)
    ]
...
def _update_state_summaries(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    updated["evaluation_inventory"] = _evaluation_inventory_from_evidence(updated)
    if globals().get("_REVIEW_ISSUE_BUNDLE_ENABLED", True):
        updated["review_issue_materialization"] = _add_reviewer_absence_audit_artifacts(
            updated,
            updated.get("claim_requirement_audit") or {},
        )
        updated["review_issue_sync"] = _sync_verified_review_issues(updated)
    return updated
...
def normalize_review_update_payload(payload: Any, required_fields: Optional[Iterable[str]] = None) -> Dict[str, Any]:
    ...
    raw_reviewer_negative_candidates: List[Any] = []
    for candidate_field in ("reviewer_negative_candidates", "review_issue_candidates"):
        candidate_value = payload.get(candidate_field)
    ...
    evidence_json_raw_head = _normalize_text(payload.get("evidence_json_raw_head"), max_length=360)
    evidence_json_raw_tail = _normalize_text(payload.get("evidence_json_raw_tail"), max_length=360)
    ...
    normalized_payload = {
        ...
        "reviewer_negative_candidates": reviewer_negative_candidates[:12],
        ...
        "missing_negative_types": _normalize_list_of_strings(payload.get("missing_negative_types")),
        ...
        "diagnosis_pending_concern": payload.get("diagnosis_pending_concern") if isinstance(payload.get("diagnosis_pending_concern"), dict) else {},
        ...
        "evidence_json_raw_head": evidence_json_raw_head,
        "evidence_json_raw_tail": evidence_json_raw_tail,
        ...
        "negative_quote_bank_salvage_blocked": bool(payload.get("negative_quote_bank_salvage_blocked", False)),
        "negative_quote_bank_salvage_block_reason": _normalize_text(
            payload.get("negative_quote_bank_salvage_block_reason"),
            max_length=120,
        ),
        "targeted_negative_search_not_assessable": copy.deepcopy(
            payload.get("targeted_negative_search_not_assessable")
        )
        if isinstance(payload.get("targeted_negative_search_not_assessable"), dict)
        else {},
        "targeted_negative_search_not_assessable_count": max(
            0,
            int(payload.get("targeted_negative_search_not_assessable_count") or 0)
            if str(payload.get("targeted_negative_search_not_assessable_count") or "").strip().isdigit()
            else 0,
        ),
    }
...
def normalize_manager_payload(payload: Any, available_agents: Optional[Iterable[str]] = None) -> Dict[str, Any]:
    ...
    targeted_negative_search_required = bool(payload.get("targeted_negative_search_required", False))
    freeform_reviewer_negative_discovery_required = bool(
        payload.get("freeform_reviewer_negative_discovery_required", False)
    )
    freeform_reviewer_negative_enabled = bool(payload.get("freeform_reviewer_negative_enabled", False))
    freeform_reviewer_negative_stage = _normalize_text(
        payload.get("freeform_reviewer_negative_stage"),
        max_length=80,
    )
    freeform_reviewer_negative_mode = _normalize_text(
        payload.get("freeform_reviewer_negative_mode"),
        max_length=120,
    )
    targeted_negative_search_active_tasks = _prompt_targeted_negative_tasks(
        payload.get("targeted_negative_search_active_tasks") or [],
        max_items=3,
    )
    ...
    evidence_json_raw_head = _normalize_text(
        payload.get("evidence_json_raw_head"), max_length=360
    )
    evidence_json_raw_tail = _normalize_text(
        payload.get("evidence_json_raw_tail"), max_length=360
    )
    evidence_json_partial_recovery = bool(payload.get("evidence_json_partial_recovery", False))
    evidence_json_fallback_payload_used = bool(payload.get("evidence_json_fallback_payload_used", False))
    ...
    return {
        **normalized_payload,
        ...
        "targeted_negative_search_required": targeted_negative_search_required,
        "freeform_reviewer_negative_discovery_required": freeform_reviewer_negative_discovery_required,
        "freeform_reviewer_negative_enabled": freeform_reviewer_negative_enabled,
        "freeform_reviewer_negative_stage": freeform_reviewer_negative_stage,
        "freeform_reviewer_negative_mode": freeform_reviewer_negative_mode,
        "targeted_negative_search_task_count": _safe_int(payload.get("targeted_negative_search_task_count", 0)),
        "targeted_negative_search_active_tasks": targeted_negative_search_active_tasks,
...
def build_review_task(extras: Dict[str, Any], mode: str, max_turns: int) -> Dict[str, Any]:
    paper_text = _normalize_paper_text(
        ...
        max_length=FULL_PAPER_TEXT_STATE_MAX_CHARS,
    )
    ...
    review_state["paper_text"] = paper_text
...
def _build_recovery_state_delta(before: Dict[str, Any], after: Dict[str, Any]) -> Dict[str, Any]:
    ...
    diagnosis_pending_concern_delta = (
        len(after.get("diagnosis_pending_concerns", []) or [])
        - len(before.get("diagnosis_pending_concerns", []) or [])
    )
    ...
    return {
        ...
        "diagnosis_pending_concern_delta": diagnosis_pending_concern_delta,
        "diagnosis_pending_concern_added": diagnosis_pending_concern_delta > 0,
    }
...
def _recovery_patch_operation(parsed_patch: Dict[str, Any], validation: Dict[str, Any]) -> str:
    ...
    if target_type == "claim_requirement_gap" and validation.get("diagnosis_pending_concern"):
        return "record_diagnosis_pending_concern"
...
def _recovery_patch_target_gate_label(parsed_patch: Dict[str, Any], validation: Dict[str, Any]) -> str:
    ...
    if target_type == "claim_requirement_gap" and validation.get("diagnosis_pending_concern"):
        return "diagnosis_pending_target"
...
def _build_recovery_patch_log(parsed_patch: Dict[str, Any], validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(relation_payload, dict):
        relation_payload = {}
    diagnosis_payload = parsed_patch.get("diagnosis_pending_concern") or raw_payload.get("diagnosis_pending_concern") or {}
    ...
    return {
        ...
        "missing_negative_types": list(validation.get("missing_negative_types", parsed_patch.get("missing_negative_types", [])) or []),
        ...
        "diagnosis_pending_concern_claim_id": str(
            validation.get("claim_requirement_gap_claim_id")
            or diagnosis_payload.get("claim_id")
            or ""
        ),
        "diagnosis_pending_concern_missing_requirements": list(
            validation.get("missing_requirements")
            or diagnosis_payload.get("missing_requirements")
            or parsed_patch.get("missing_requirements")
            or []
        ),
        "diagnosis_pending_concern_missing_negative_types": list(
            validation.get("missing_negative_types")
            or diagnosis_payload.get("missing_negative_types")
            or parsed_patch.get("missing_negative_types")
            or []
        ),
        "diagnosis_pending_concern_final_view": str(diagnosis_payload.get("final_view") or ""),
        "diagnosis_pending_concern_status": str(diagnosis_payload.get("status") or ""),
...
def _flaw_limitation_patch_is_no_effect(state: Dict[str, Any], validation: Dict[str, Any], new_status: str) -> bool:
    ...
    flaw_record = next(
        (
            item for item in state.get("flaw_candidates", []) or []
            if isinstance(item, dict) and str(item.get("flaw_id") or "").strip() == target_id
        ),
        None,
    )
    evidence_by_id = {
    ...
    supporting_ids = [str(item or "").strip() for item in validation.get("supporting_evidence_ids", []) or [] if str(item or "").strip()]
    if flaw_record and supporting_ids:
        related_claim_ids = [
            str(item or "").strip()
            for item in flaw_record.get("related_claim_ids", []) or []
            if str(item or "").strip()
        ]
        ...
        has_real_negative_claim = any(_is_real_paper_claim_id_for_negative(claim_id) for claim_id in dict.fromkeys(related_claim_ids))
        supporting_types = {
            _negative_evidence_type_for_record(evidence_by_id.get(evidence_id, {}))
            for evidence_id in supporting_ids
        }
        has_actionable_supporting_negative = any(
            neg_type in ACTIONABLE_NEGATIVE_EVIDENCE_TYPES
            and neg_type not in LIMITATION_NEGATIVE_EVIDENCE_TYPES
            for neg_type in supporting_types
        )
        if related_claim_ids and not has_real_negative_claim and not has_actionable_supporting_negative:
            return True
    try:
    ...
    for flaw in view.get("flaw_candidates", []) or []:
        ...
        if layer != "assessment_limitation":
            return False
        if not conflicts:
            return True
        return all(_negative_grounding_conflict_is_semantic_rejection(conflict) for conflict in conflicts)
    return False
...
def _mark_contested_references_absence_audit_evidence(parsed_patch: Dict[str, Any]) -> bool:
    raw_payload = parsed_patch.get("raw_payload") or {}
    operation = str(
        parsed_patch.get("recovery_patch_operation")
        or raw_payload.get("recovery_patch_operation")
        or ""
    ).strip()
    if operation != "mark_contested" and not (parsed_patch.get("mark_contested") or raw_payload.get("mark_contested")):
        return False
    relation_payload = parsed_patch.get("contested_relation") or raw_payload.get("contested_relation") or {}
    raw_ids: List[Any] = []
    raw_ids.extend(parsed_patch.get("supporting_evidence_ids", []) or [])
    raw_ids.extend((relation_payload or {}).get("negative_evidence_ids", []) or [])
    return any(str(item or "").strip().startswith("evidence-reviewer-absence-") for item in raw_ids)


def _recovery_absence_audit_evidence_snapshots(
    validation_state: Dict[str, Any],
    parsed_patch: Dict[str, Any],
) -> List[Dict[str, Any]]:
    if not _mark_contested_references_absence_audit_evidence(parsed_patch):
        return []
    raw_payload = parsed_patch.get("raw_payload") or {}
    relation_payload = parsed_patch.get("contested_relation") or raw_payload.get("contested_relation") or {}
    raw_ids: List[Any] = []
    ...
    evidence_lookup = {
        str(item.get("evidence_id") or ""): item
        for item in (validation_state or {}).get("evidence_map", []) or []
        if isinstance(item, dict) and str(item.get("evidence_id") or "")
    }
    ...
    for evidence_id in dict.fromkeys(requested_ids):
        ...
        if not isinstance(evidence, dict):
            continue
        if not _is_reviewer_absence_audit_evidence_record(evidence, validation_state or {}):
            continue
        snapshot = copy.deepcopy(evidence)
        snapshot["absence_audit_snapshot_at_recovery_commit"] = True
        snapshot.setdefault("recovery_snapshot_source", "mark_contested")
...
def _apply_recovery_update(state: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if str(parsed_patch.get("target_type") or "").strip().lower() == "claim_requirement_gap":
        validation_state = copy.deepcopy(state or {})
        validation_state["claim_requirement_audit"] = _claim_requirement_audit(validation_state)
    elif _mark_contested_references_absence_audit_evidence(parsed_patch):
        validation_state = copy.deepcopy(state or {})
        validation_state.pop("decision_hygiene", None)
        validation_state = build_decision_hygiene_view(validation_state)
    validation = validate_recovery_patch(validation_state, parsed_patch)
    absence_audit_snapshots = _recovery_absence_audit_evidence_snapshots(validation_state, parsed_patch)
    ...
    if absence_audit_snapshots:
        ...
        new_snapshots = [
            item for item in absence_audit_snapshots
            if str(item.get("evidence_id") or "") not in existing_ids
        ]
        if new_snapshots:
            merged["evidence_map"] = list(merged.get("evidence_map", []) or []) + new_snapshots
    ...
    record_diagnosis_pending_concern = bool(
        patch_operation == "record_diagnosis_pending_concern"
        and validation.get("diagnosis_pending_concern")
    )
    ...
    try:
        if relation_only_mark_contested or record_diagnosis_pending_concern:
            pass
    ...
    if record_diagnosis_pending_concern:
        gap_item = validation.get("target_item") or {}
        raw_payload = parsed_patch.get("raw_payload") or {}
        concern_payload = parsed_patch.get("diagnosis_pending_concern") or raw_payload.get("diagnosis_pending_concern") or {}
        if not isinstance(concern_payload, dict):
            concern_payload = {}
        claim_id = str(
            validation.get("claim_requirement_gap_claim_id")
            or concern_payload.get("claim_id")
            or gap_item.get("claim_id")
            or parsed_patch.get("target_id")
            or ""
        ).strip()
        missing_requirements = [
            ...
            for item in (
                validation.get("missing_requirements")
                or concern_payload.get("missing_requirements")
                or gap_item.get("missing_requirements")
                or []
            )
            if str(item or "").strip()
        ]
        missing_negative_types = [
            ...
            for item in (
                validation.get("missing_negative_types")
                or concern_payload.get("missing_negative_types")
                or gap_item.get("missing_negative_types")
                or []
            )
            if str(item or "").strip()
        ]
        if claim_id and missing_requirements:
            existing = [
                item for item in merged.get("diagnosis_pending_concerns", []) or []
                if isinstance(item, dict)
            ]
            existing_keys = {
                (
                    str(item.get("claim_id") or "").strip(),
                    tuple(str(req or "").strip() for req in item.get("missing_requirements", []) or [] if str(req or "").strip()),
                )
                for item in existing
            }
            concern_key = (claim_id, tuple(missing_requirements))
            if concern_key not in existing_keys:
                concern = {
                    "concern_id": _slugify(
                        "diagnosis-pending",
                        f"{claim_id}-{'-'.join(missing_requirements)}",
                        len(existing) + 1,
                    ),
                    "claim_id": claim_id,
                    "claim": _normalize_text(gap_item.get("claim") or concern_payload.get("claim"), max_length=240),
                    "missing_requirements": missing_requirements[:6],
                    "missing_negative_types": missing_negative_types[:6],
                    "reason": (
                        parsed_patch.get("reason_for_change")
                        or concern_payload.get("reason")
                        or "Claim requirement audit found missing verified support coverage."
                    ),
                    "final_view": str(concern_payload.get("final_view") or "potential_concern"),
                    "status": "recorded",
                    "grounding_status": "diagnosis_pending_verification",
                    "basis": str(validation.get("audit_basis") or gap_item.get("audit_basis") or "claim_requirement_vs_verified_support"),
                    "source": "claim_requirement_audit",
                }
                merged["diagnosis_pending_concerns"] = (existing + [concern])[-12:]
                merged["_latest_patch_log"].update(
                    {
                        "diagnosis_pending_concern_claim_id": claim_id,
                        "diagnosis_pending_concern_missing_requirements": concern["missing_requirements"],
                        "diagnosis_pending_concern_missing_negative_types": concern["missing_negative_types"],
                        "diagnosis_pending_concern_final_view": concern["final_view"],
                        "diagnosis_pending_concern_status": concern["status"],
                    }
                )
...
def merge_review_state(state: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged["claims"], claim_revisions, claim_lifecycle_conflicts = _merge_items_with_revisions(
        ...
        key="claim_id",
        entity_type="claim",
        tracked_fields=("claim", "importance", "status", "claim_type", "claim_kind", "claim_origin", "claim_origin_kind", "claim_source", "claim_extraction_source", "evidence_need", "coverage_tags", "claim_obligations", "supporting_evidence_ids"),
        max_items=8,
    )
    ...
    normalized_payload["evidence_map"] = _validate_evidence_bindings_for_state(
        merged,
        [
            _strip_model_claimed_evidence_verification_fields(item)
            for item in normalized_payload.get("evidence_map", []) or []
            if isinstance(item, dict)
        ],
    )
    ...
    merged["flaw_candidates"], flaw_revisions, flaw_lifecycle_conflicts = _merge_items_with_revisions(
        ...
        key="flaw_id",
        entity_type="flaw",
        tracked_fields=("title", "description", "severity", "status", "related_claim_ids", "evidence_ids", "negative_evidence_ids", "verified_negative_evidence_ids", "grounding_status", "weakness_type", "required_evidence_type", "criterion", "paper_side_rationale", "source_stage", "source"),
        max_items=8,
    )
    ...
    merged["reviewer_negative_candidates"], candidate_revisions, candidate_lifecycle_conflicts = _merge_items_with_revisions(
        merged.get("reviewer_negative_candidates", []),
        normalized_payload.get("reviewer_negative_candidates", []),
        key="candidate_id",
        entity_type="reviewer_negative_candidate",
        tracked_fields=(
            "claim_id",
            "claim",
            "weakness",
    ...
    revision_events.extend(candidate_revisions)
    conflict_events.extend(candidate_lifecycle_conflicts)
...
def _manager_requires_negative_evidence_formation(manager_payload: Dict[str, Any]) -> bool:
    payload = manager_payload or {}
    # A hard-negative *diagnosis* turn (DRMAS_HARDNEG_DIAGNOSIS on) is NOT a negative-formation turn:
    # it sets negative_evidence_formation_required=False and routes Critique to analyze_flaws. Without
    # this guard it would be miscounted as formation because it reuses the hard_negative_discovery_override
    # policy_source.
    if payload.get("hard_negative_diagnosis_required"):
        return False
    policy_source = str(payload.get("policy_source") or "").strip()
    if policy_source == "targeted_negative_search_override" and not payload.get("targeted_negative_search_required"):
        return False
    return bool(
        payload.get("negative_evidence_formation_required")
        or policy_source in {"negative_evidence_formation_override", "hard_negative_discovery_override", "targeted_negative_search_override", "compact_negative_pass_override"}
    )
...
def compact_review_state_for_prompt(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "evaluation_inventory": {
            "items": list(((state.get("evaluation_inventory") or {}).get("items") or []))[:8]
            if isinstance(state.get("evaluation_inventory"), dict)
            else [],
            "type_counts": dict(((state.get("evaluation_inventory") or {}).get("type_counts") or {}))
            if isinstance(state.get("evaluation_inventory"), dict)
            else {},
        },
        "flaw_candidates": state.get("flaw_candidates", [])[:5],
        "unresolved_questions": state.get("unresolved_questions", [])[:6],
        "reviewer_negative_candidates": state.get("reviewer_negative_candidates", [])[:6],
...
_EVIDENCE_NEGATIVE_ANCHOR_PATTERNS: List[Tuple[str, re.Pattern[str]]] = [
    (
        ...
        re.compile(
            ...
            r"missing\s+(?:ablation|baseline|comparison|evaluation|implementation|hyperparameter|details?|runtime|latency|memory|compute|cost)|"
            r"not\s+(?:evaluated|compared|significant|consistent|reproducible)|no\s+significant|"
            r"without\s+(?:an?\s+|the\s+)?(?:baseline|comparison|robustness|generalization|runtime|latency|memory|compute|cost)(?:\s+(?:to|with|against))?|"
            r"(?:do|does|did)\s+not\s+(?:report|provide|include|evaluate|compare|validate|establish|show|measure)\b|"
            r"lack(?:s|ed|ing)?\s+(?:ablation|baseline|comparison|evaluation|implementation|detail|hyperparameter|component analysis|robustness|generalization|runtime|latency|memory|compute|cost)|"
            r"insufficient\s+(?:evaluation|experiment|baseline|comparison|detail|implementation|robustness|generalization)|"
            r"single\s+dataset|only\s+evaluated|limited\s+to\s+(?:one|two|a single)|"
            r"unfair\s+(?:comparison|baseline)|weak\s+baseline|oracle|leakage|train[- ]test|test[- ]set|validation[- ]set|"
...
_NEG_TYPE_UNFAIR_OR_WEAK_BASELINE_RE = re.compile(
    r"\b(unfair (?:comparison|baseline|evaluation)|weak baseline|weak baselines|"
    r"baseline(?:s)? (?:are|is|was|were)?\s*(?:weak|outdated|not tuned|untuned|unfair|inadequate)|"
    r"(?:uses?|used|chooses?|chosen|compares? against) (?:a |an |the )?(?:weak|outdated|untuned|non[- ]competitive) baseline|"
    r"not (?:tuned|optimized) (?:for|on) (?:the )?(?:baseline|baselines)|"
    r"different (?:backbone|architecture|training budget|data split|protocol|setting) (?:for|between) (?:the )?(?:baseline|baselines|methods)|"
    r"(?:baseline|comparison)[^.!?]{0,120}(?:unfair|weak|outdated|untuned|not tuned|different (?:backbone|training budget|protocol|setting)))\b",
    re.IGNORECASE,
)
...
_NEG_TYPE_MISSING_ROBUSTNESS_GENERALIZATION_RE = re.compile(
    r"\b(no (?:robustness|generalization|generalisation|out-of-domain|ood|cross-domain|stress) (?:test|evaluation|experiment|benchmark)|"
    r"missing (?:robustness|generalization|generalisation|out-of-domain|ood|cross-domain|stress) (?:test|evaluation|experiment|benchmark)|"
    r"(?:robustness|generalization|generalisation|transferability|out-of-domain|ood|cross-domain)[^.!?]{0,120}(?:not (?:tested|evaluated|validated)|missing|absent|limited)|"
...
_NEG_TYPE_EVALUATION_PROTOCOL_RISK_RE = re.compile(
    r"\b(train[- ]test leakage|data leakage|test[- ]set leakage|label leakage|"
    r"oracle (?:information|access|features?|labels?)|uses? (?:the )?test set (?:for|to) (?:tuning|select|selection|validation)|"
...
_NEG_TYPE_EFFICIENCY_COST_GAP_RE = re.compile(
    ...
    r"high (?:runtime|latency|memory|computational cost|compute cost)|"
    r"computationally expensive|requires? substantial compute|requires? large memory|"
    r"(?:scalability|scalable|efficient|efficiency)[^.!?]{0,120}(?:not (?:evaluated|reported|measured)|missing|unclear))\b",
    re.IGNORECASE,
)
...
def _classify_negative_evidence_type(quote: str) -> str:
    ...
    if _NEG_TYPE_MISSING_ABLATION_RE.search(quote):
        return "missing_ablation"
    if _NEG_TYPE_UNFAIR_OR_WEAK_BASELINE_RE.search(quote):
        return "unfair_or_weak_baseline"
    if _NEG_TYPE_MISSING_BASELINE_RE.search(quote):
        return "missing_baseline"
    if _NEG_TYPE_EVALUATION_PROTOCOL_RISK_RE.search(quote):
        return "evaluation_protocol_risk"
    if _NEG_TYPE_MISSING_ROBUSTNESS_GENERALIZATION_RE.search(quote):
        return "missing_robustness_or_generalization"
    if _NEG_TYPE_INSUFFICIENT_EVALUATION_RE.search(quote):
        return "insufficient_evaluation"
    if _NEG_TYPE_EFFICIENCY_COST_GAP_RE.search(quote):
        return "efficiency_cost_gap"
...
NEGATIVE_EVIDENCE_TYPES_ALL = frozenset(
    {"direct_contradiction", "negative_result", "missing_ablation", "missing_baseline",
     "unfair_or_weak_baseline", "insufficient_evaluation", "missing_robustness_or_generalization",
     "evaluation_protocol_risk", "efficiency_cost_gap",
     "reproducibility_gap", "method_support_gap", "scope_overclaim", "result_claim_mismatch",
...
LIMITATION_NEGATIVE_EVIDENCE_TYPES = frozenset({"scope_limitation", "generic_gap"})
TRUE_PAPER_NEGATIVE_EVIDENCE_TYPES = frozenset(
    {
        "direct_contradiction",
        "negative_result",
        "missing_ablation",
...
SECONDARY_NEGATIVE_EVIDENCE_TYPES = frozenset({"scope_overclaim", "scope_limitation", "generic_gap"})


def _negative_evidence_type_priority(negative_type: str) -> int:
    """Rank paper-side negative types by diagnostic value.

    True negatives are paper flaws or direct claim-support failures.  Scope /
    limitation-style signals are kept as fallback context, but they should not
    displace concrete missing-evaluation, result-mismatch, or contradiction
    evidence in the Critique prompt.
    """
    value = str(negative_type or "").strip()
    if value in TRUE_PAPER_NEGATIVE_EVIDENCE_TYPES:
        return 0
    if value == "scope_overclaim":
        return 1
    if value in {"scope_limitation", "generic_gap"}:
        return 2
    return 3


def _negative_evidence_type_for_record(record: Dict[str, Any]) -> str:
    explicit = _canonical_negative_evidence_type((record or {}).get("negative_evidence_type"))
...
# When OFF (default), the Critique state slice carries neither the legacy live claim_requirement_gaps
# (kept decoupled) nor the new hard_negative_diagnosis_targets block, so default Critique behavior
# matches the validated mainline. When ON, the claim-centric model-judgment targets are injected for
# Mac multi-seed A/B. Pairs with review_prompts._HARDNEG_DIAGNOSIS_ENABLED (same env var), which
# gates the matching critique prompt additions.
...
_TARGETED_NEGATIVE_QUERY_STOPWORDS = _CLAIM_QUERY_STOPWORDS | {
    "against",
    "candidate",
    "check",
    "claim",
...
_TARGETED_NEGATIVE_REQUIREMENT_TERMS = {
    "baseline_or_comparison": {"baseline", "baselines", "comparison", "compare", "compared", "sota", "state-of-the-art", "against"},
    "ablation_or_component": {"ablation", "ablations", "component", "module", "variant", "without"},
    "empirical_result": {"experiment", "experiments", "evaluation", "result", "results", "table", "benchmark", "accuracy", "performance"},
    "scope_coverage": {"dataset", "datasets", "task", "tasks", "domain", "domains", "generalization", "robustness"},
    "robustness_or_generalization": {"robustness", "generalization", "ood", "cross-domain", "stress", "dataset", "datasets"},
    "efficiency_cost": {"runtime", "latency", "throughput", "speedup", "memory", "compute", "cost", "tokens"},
    "reproducibility_detail": {"implementation", "hyperparameter", "training", "details", "code", "reproduce"},
    "method_detail": {"method", "algorithm", "architecture", "training", "implementation", "details"},
}


def _targeted_negative_task_query_terms(task_item: Dict[str, Any]) -> set[str]:
    ...
    for key in ("expected_quote_cues", "missing_or_weak_items", "coverage_missing_items", "coverage_observed_items"):
        values = task_item.get(key)
    ...
    parts.extend(_TARGETED_NEGATIVE_REQUIREMENT_TERMS.get(requirement, set()))
    text = " ".join(parts)
    terms = {
        token
        for token in re.findall(r"[A-Za-z][A-Za-z0-9_-]{2,}", text.lower())
        if token not in _TARGETED_NEGATIVE_QUERY_STOPWORDS and not token.isdigit()
    }
    return set(list(terms)[:48])


def _targeted_negative_task_phrases(task_item: Dict[str, Any]) -> List[str]:
    phrases: List[str] = []
    for key in ("expected_quote_cues", "missing_or_weak_items", "coverage_missing_items", "coverage_observed_items"):
        values = task_item.get(key) if isinstance(task_item, dict) else None
        if isinstance(values, list):
            phrases.extend(str(item).strip() for item in values if str(item or "").strip())
    for key in ("target_locator_hint", "reviewer_negative_candidate", "search_question", "candidate_raw_quote"):
        value = str((task_item or {}).get(key) or "").strip()
...
def _iter_limited_phrase_matches(body: str, phrase: str, *, start_pos: int = 0, max_matches: int = 4) -> List[int]:
    ...
    return [match.start() for match in pattern.finditer(body, pos=max(0, start_pos))][:max_matches]


def _candidate_relevant_paper_windows(
    body: str,
    ...
    *,
    ...
    if not body or not tasks:
        return []
    scored: List[Tuple[int, int, int, Dict[str, Any]]] = []
    seen_window_keys: set[str] = set()
    for task_index, task_item in enumerate(item for item in tasks if isinstance(item, dict)):
        query_terms = _targeted_negative_task_query_terms(task_item)
        phrases = _targeted_negative_task_phrases(task_item)
        requirement = str(task_item.get("required_evidence_type") or "").strip()
        negative_type = str(task_item.get("negative_type") or "").strip()
        positions: List[Tuple[int, str]] = []
        for phrase in phrases[:12]:
            for pos in _iter_limited_phrase_matches(body, phrase, start_pos=fallback_start, max_matches=4):
                positions.append((pos, phrase))
        for term in sorted(query_terms, key=lambda item: (-len(item), item))[:18]:
            for pos in _iter_limited_phrase_matches(body, term, start_pos=fallback_start, max_matches=3):
                positions.append((pos, term))
        for source in _TARGETED_NEGATIVE_REQUIREMENT_SOURCE_ORDER.get(requirement, []):
            for anchor_source, pattern in list(_EVIDENCE_DETAIL_ANCHOR_PATTERNS) + list(_EVIDENCE_NEGATIVE_ANCHOR_PATTERNS):
                if anchor_source != source:
                    continue
                for match in pattern.finditer(body, pos=fallback_start):
                    positions.append((match.start(), source))
                    if len([1 for _, marker in positions if marker == source]) >= 3:
                        break
        if not positions:
            continue
        for pos, marker in positions[:80]:
            window_size = 980
            span_start = max(0, pos - window_size // 3)
            span_end = min(len(body), pos + window_size)
            snippet = _window_around(body, pos, window=window_size)
            if len(snippet) < 80:
                continue
            window_key = _quote_bank_dedupe_key(snippet[:520])
            if len(window_key) >= 40 and window_key in seen_window_keys:
                continue
            snippet_l = snippet.lower()
            term_hits = _claim_overlap_score(snippet, query_terms)
            phrase_hits = sum(1 for phrase in phrases if phrase.lower() in snippet_l)
            source_hits = sum(1 for source in _TARGETED_NEGATIVE_REQUIREMENT_SOURCE_ORDER.get(requirement, []) if source in snippet_l)
            table_hits = len(re.findall(r"\b(?:table|figure|fig\.)\s*\d+|\d+(?:\.\d+)?\s*%", snippet, re.IGNORECASE))
            ablation_bonus = 4 if requirement == "ablation_or_component" and re.search(r"\bablat|without|variant|component|module\b", snippet, re.IGNORECASE) else 0
            comparison_bonus = 4 if requirement == "baseline_or_comparison" and re.search(r"\bbaseline|compare|comparison|sota|state-of-the-art\b", snippet, re.IGNORECASE) else 0
            score = term_hits * 3 + phrase_hits * 8 + source_hits * 2 + min(table_hits, 4) + ablation_bonus + comparison_bonus
            if score <= 0:
                continue
            seen_window_keys.add(window_key)
            matched_terms = sorted(
                {term for term in query_terms if re.search(rf"\b{re.escape(term)}\b", snippet, re.IGNORECASE)},
                key=lambda item: (-len(item), item),
            )[:10]
            matched_phrases = [phrase for phrase in phrases if phrase.lower() in snippet_l][:6]
            scored.append((
                -score,
                task_index,
                pos,
                {
                    "window_id": f"candidate-window-{len(scored) + 1}",
                    "task_id": str(task_item.get("task_id") or ""),
                    "claim_id": str(task_item.get("claim_id") or ""),
                    "negative_type": negative_type,
                    "required_evidence_type": requirement,
                    "score": score,
                    "matched_terms": matched_terms,
                    "matched_cues": matched_phrases,
                    "window_text": _truncate_snippet_text(snippet, 760),
    ...
    for _, _, _, item in sorted(scored, key=lambda row: (row[0], row[1], row[2])):
        task_id = str(item.get("task_id") or "")
        if task_id and task_id in used_tasks and len(windows) < max_windows - 1:
            continue
        windows.append(item)
        if task_id:
    ...
    for idx, item in enumerate(windows, start=1):
        item["window_id"] = f"candidate-window-{idx}"
    return windows


def _candidate_window_quote_bank_entries(windows: Sequence[Dict[str, Any]], max_items: int = 3) -> List[Dict[str, Any]]:
    entries: List[Dict[str, Any]] = []
    for idx, window in enumerate(windows[:max_items], start=1):
        if not isinstance(window, dict):
            continue
        raw_quote = _normalize_text(window.get("window_text"), max_length=760)
        if len(_semantic_tokens(raw_quote)) < 8:
            continue
        task_id = str(window.get("task_id") or "")
        neg_type = str(window.get("negative_type") or "").strip()
        source_start = _normalize_int(window.get("source_span_start"), default=-1)
        source_end = _normalize_int(window.get("source_span_end"), default=-1)
        entries.append(
            {
                "quote_id": f"quote-candidate-window-{idx}",
                "source_bucket": "candidate_window",
                "source_locator": f"Candidate negative window #{idx}",
                "raw_quote": raw_quote,
                "source_span_start": source_start,
                "source_span_end": source_end,
                "claim_overlap_score": 0,
                "negative_evidence_type": neg_type,
                "targeted_negative_search_task_id": task_id,
                "candidate_window_quote": True,
                "support_role_hint": "review_negative_search_window",
                "copy_rule": (
                    "Copy only the exact sentence or table fragment from raw_quote that grounds the reviewer-negative task; "
                    "do not copy the whole window unless the whole window is needed."
                ),
            }
        )
    return entries
...
def _build_critique_negative_quote_bank(body: str, max_quotes: int = 6) -> List[Dict[str, Any]]:
    ...
    for _, pattern in _EVIDENCE_NEGATIVE_ANCHOR_PATTERNS:
        for match in pattern.finditer(body, pos=first_nonabstract_pos):
            ...
            if _is_low_quality_negative_quote(quote):
                continue
            score = _negative_evidence_type_priority(neg_type)
    ...
    for _, pos, quote in ordered_candidates:
        ...
        quote_bank.append({
            ...
            "negative_evidence_actionability": (
                "true_paper_negative" if neg_type in TRUE_PAPER_NEGATIVE_EVIDENCE_TYPES
                else "secondary_scope_or_limitation"
            ),
...
def _targeted_negative_preferred_sources(tasks: Optional[Sequence[Dict[str, Any]]]) -> List[str]:
    preferred: List[str] = ["candidate_window"] if tasks else []
...
def _render_evidence_context_with_meta(
    ...
    if targeted_negative_mode:
        candidate_windows = _candidate_relevant_paper_windows(
            body,
            targeted_negative_tasks,
            fallback_start=fallback_start,
            max_windows=3,
        )
        for window in candidate_windows:
            window_text = str(window.get("window_text") or "").strip()
            if window_text:
                snippets.append(("candidate_window", window_text))
    ...
    quote_bank = candidate_window_quote_bank + _build_evidence_quote_bank(
        body,
        max_quotes=max(6, 12 - len(candidate_window_quote_bank)),
        claim_query_terms=claim_query_terms,
    )
    ...
    meta = {
        ...
        "targeted_negative_candidate_window_count": len(candidate_windows) if targeted_negative_mode else 0,
        "targeted_negative_candidate_windows": candidate_windows[:3] if targeted_negative_mode else [],
...
def _render_evidence_state_slice(
    state: Dict[str, Any],
    target_claim_ids: Optional[Sequence[str]] = None,
    target_evidence_ids: Optional[Sequence[str]] = None,
    target_flaw_ids: Optional[Sequence[str]] = None,
    action_type: str = "verify_evidence",
    targeted_negative_search_required: bool = False,
) -> Dict[str, Any]:
    ...
    targeted_negative_search_tasks = (
        _targeted_negative_search_tasks(state, claims=claims, max_items=2)
        if targeted_negative_search_required and _TARGETED_NEGATIVE_SEARCH_ENABLED
        else []
    )
    evidence_slice = {
        "active_focus": state.get("active_focus", state.get("last_focus", "")),
    ...
    return evidence_slice


def _render_critique_state_slice(
    state: Dict[str, Any],
    target_claim_ids: Optional[Sequence[str]] = None,
    target_flaw_ids: Optional[Sequence[str]] = None,
    target_evidence_ids: Optional[Sequence[str]] = None,
    action_type: str = "analyze_flaws",
    review_issue_discovery_required: bool = False,
) -> Dict[str, Any]:
    ...
    if action_type == "challenge_previous_hypothesis":
        ...
        for item in state.get("evidence_map", []) or []:
            if not isinstance(item, dict):
                continue
            if _is_grounded_paper_negative_evidence_record(item, state):
    ...
    negative_evidence = [item for item in evidence if _is_grounded_paper_negative_evidence_record(item, state)]
    negative_binding = _negative_evidence_binding_view(state)
    ...
    # Gated, default off: only surface the claim-centric diagnosis targets when the
    # hard-negative diagnosis experiment is enabled. When off, the slice matches the
    # validated mainline (no claim_requirement_gaps live-wiring, no diagnosis targets).
    if _HARDNEG_DIAGNOSIS_ENABLED:
        critique_slice["hard_negative_diagnosis_targets"] = _hard_negative_diagnosis_targets(state, claims=claims, max_items=4)
        critique_slice["hard_negative_diagnosis_rule"] = (
            "Critique must first judge real paper-side weaknesses against these claim-level checks. "
    ...
    if review_issue_discovery_required and _REVIEW_ISSUE_BUNDLE_ENABLED:
        critique_slice["review_issue_discovery_targets"] = _hard_negative_diagnosis_targets(state, claims=claims, max_items=6)
        critique_slice["review_issue_discovery_rule"] = (
            "Critique proposes review_issue_candidates only. A candidate becomes verified only after claim-obligation/support-inventory audit "
            "or copied-quote verification; model judgment alone is never counted as verified negative evidence."
        )
    return critique_slice
...
def _prompt_targeted_negative_quote_entries(quote_bank: Sequence[Dict[str, Any]], max_items: int = 4) -> List[Dict[str, Any]]:
    entries: List[Dict[str, Any]] = []
    for item in quote_bank[:max_items]:
        if not isinstance(item, dict):
            continue
        raw_quote = _normalize_text(item.get("raw_quote"), max_length=140)
        if not raw_quote:
            continue
        negative_type = _normalize_text(
            item.get("negative_evidence_type") or _classify_negative_evidence_type(raw_quote),
            max_length=80,
        )
        entries.append({
            "quote_id": str(item.get("quote_id") or ""),
            "source_locator": str(item.get("source_locator") or ""),
            "negative_hint": negative_type,
            "raw_quote": raw_quote,
        })
    return entries
...
def _prompt_targeted_negative_tasks(tasks: Sequence[Dict[str, Any]], max_items: int = 1) -> List[Dict[str, Any]]:
    ...
    for item in tasks[:max_items]:
        ...
        compact.append({
            "task_id": str(item.get("task_id") or ""),
            "claim_id": str(item.get("claim_id") or ""),
            "claim": _normalize_text(item.get("claim"), max_length=90),
            "negative_type": str(item.get("negative_type") or ""),
            "required_evidence_type": str(item.get("required_evidence_type") or ""),
            "search_question": _normalize_text(item.get("search_question"), max_length=90),
            "quote_cues": _normalize_list_of_strings(item.get("expected_quote_cues"), max_items=3, max_length=24),
            "missing_or_weak_items": _normalize_list_of_strings(
                item.get("missing_or_weak_items"),
                max_items=5,
                max_length=60,
            ),
            "locator_hint": _normalize_text(item.get("target_locator_hint"), max_length=80),
            "quote_id": str(item.get("quote_id") or ""),
            "candidate_raw_quote": _normalize_text(item.get("candidate_raw_quote"), max_length=160),
            "quote_grounding_mode": str(item.get("quote_grounding_mode") or ""),
            "reviewer_negative_candidate_id": str(item.get("reviewer_negative_candidate_id") or ""),
            "reviewer_negative_candidate": _normalize_text(item.get("reviewer_negative_candidate"), max_length=160),
            "source": str(item.get("source") or ""),
        })
    return compact


def _prompt_freeform_reviewer_negative_claims(claims: Sequence[Dict[str, Any]], max_items: int = 4) -> List[Dict[str, Any]]:
    compact: List[Dict[str, Any]] = []
    for item in claims[:max_items]:
        if not isinstance(item, dict):
            continue
        claim_id = str(item.get("claim_id") or "")
        if not claim_id:
        ...
        compact.append({
            "claim_id": claim_id,
            "claim": _normalize_text(item.get("claim") or item.get("text"), max_length=180),
            "claim_type": str(item.get("claim_type") or ""),
            "importance": str(item.get("importance") or ""),
            "status": str(item.get("status") or ""),
            "coverage_tags": _normalize_list_of_strings(item.get("coverage_tags"), max_items=5, max_length=32),
            "claim_obligations": _normalize_required_evidence_types(
                item.get("claim_obligations") or item.get("required_evidence_types"),
                max_items=5,
            ),
        })
    return compact
...
def _targeted_negative_attempted_reviewer_candidate_ids(
    turn_logs: Sequence[Dict[str, Any]],
    *,
    window: int = 8,
) -> List[str]:
    """Reviewer-negative candidate ids already routed to targeted evidence."""
    attempted: List[str] = []
    seen: set[str] = set()
    for turn in list(turn_logs or [])[-window:]:
        values: List[Any] = []
        values.extend(turn.get("targeted_negative_search_active_candidate_ids") or [])
        for task in turn.get("targeted_negative_search_active_tasks") or []:
            if isinstance(task, dict):
                values.append(task.get("reviewer_negative_candidate_id"))
        for value in values:
...
def _prompt_negative_quote_bank_entries(quote_bank: Sequence[Dict[str, Any]], max_items: int = 5) -> List[Dict[str, Any]]:
    ...
    for item in ordered_items[:max_items]:
        ...
        entries.append({
            ...
            "copy_rule": (
                "If used for a flaw, cite this quote_id in negative_evidence_ids and explain the paper-side weakness. "
                "Prefer true_paper_negative entries; use secondary scope/limitation entries only as assessment limitations or last-resort concerns."
            ),
        })
    return entries
...
def render_evidence_observation(task: Dict[str, Any], manager_payload: Optional[Dict[str, Any]] = None) -> str:
    ...
    targeted_negative_requested = bool(manager_payload.get("targeted_negative_search_required", False))
    freeform_reviewer_negative_requested = bool(
        manager_payload.get("freeform_reviewer_negative_discovery_required", False)
        and _FREEFORM_REVIEWER_NEGATIVE_ENABLED
    )
    if targeted_negative_requested:
        attempted_candidate_ids = _targeted_negative_attempted_reviewer_candidate_ids(task.get("turn_logs", []))
        if attempted_candidate_ids:
            state["_targeted_negative_attempted_reviewer_candidate_ids"] = attempted_candidate_ids
    evidence_slice = _render_evidence_state_slice(
        ...
        targeted_negative_search_required=targeted_negative_requested or freeform_reviewer_negative_requested,
    )
    ...
    evidence_context_max_length = 3200 if freeform_reviewer_negative_requested else (1800 if targeted_negative_requested else 2300)
    evidence_context, evidence_context_meta = _render_evidence_context_with_meta(
        task,
        max_length=evidence_context_max_length,
        state=state,
    ...
    if targeted_negative_requested and _TARGETED_NEGATIVE_SEARCH_ENABLED:
        state_with_current_meta = copy.deepcopy(state)
        state_with_current_meta["_latest_evidence_context_meta"] = dict(evidence_context_meta)
        current_target_claims = evidence_slice.get("target_claims") or [
            item for item in state_with_current_meta.get("claims", []) or []
            if isinstance(item, dict)
        ]
        refreshed_tasks = _targeted_negative_search_tasks(
            state_with_current_meta,
            claims=current_target_claims,
            max_items=2,
        )
        if refreshed_tasks:
            evidence_slice["targeted_negative_search_tasks"] = refreshed_tasks
            evidence_slice["targeted_negative_search_refreshed_from_current_quote_bank"] = True
            # The active targeted-negative task can be selected only after the
            # first context pass builds the current quote bank. Re-render so the
            # worker sees candidate-specific paper windows for the actual task.
            evidence_context, evidence_context_meta = _render_evidence_context_with_meta(
                task,
                max_length=evidence_context_max_length,
                state=state,
    ...
    targeted_negative_search_tasks = evidence_slice.get("targeted_negative_search_tasks", []) or []
    freeform_reviewer_negative_mode = bool(freeform_reviewer_negative_requested)
    if freeform_reviewer_negative_mode:
        evidence_slice["targeted_negative_search_tasks"] = []
        targeted_negative_search_tasks = []
    if freeform_reviewer_negative_mode:
        manager_payload["freeform_reviewer_negative_enabled"] = True
        manager_payload["freeform_reviewer_negative_mode"] = "candidate_discovery_then_quote_verification_v1"
        manager_payload["freeform_reviewer_negative_stage"] = "candidate_discovery"
    elif any(str(item.get("source") or "") == "freeform_reviewer_negative_candidate" for item in targeted_negative_search_tasks):
        manager_payload["freeform_reviewer_negative_enabled"] = True
        manager_payload["freeform_reviewer_negative_mode"] = "candidate_discovery_then_quote_verification_v1"
        manager_payload["freeform_reviewer_negative_stage"] = "quote_verification"
    manager_payload["targeted_negative_search_required"] = bool(
        manager_payload.get("targeted_negative_search_required", False)
    )
    manager_payload["targeted_negative_search_task_count"] = len(targeted_negative_search_tasks)
    manager_payload["targeted_negative_search_active_tasks"] = _prompt_targeted_negative_tasks(
        targeted_negative_search_tasks,
        max_items=2,
    )
    for key in (
    ...
    targeted_negative_mode = bool(targeted_negative_search_tasks) or freeform_reviewer_negative_mode
    if targeted_negative_requested and not targeted_negative_mode:
        manager_payload["targeted_negative_search_required"] = False
        manager_payload["negative_evidence_formation_required"] = False
        manager_payload["targeted_negative_search_no_task_blocked"] = True
        if str(manager_payload.get("policy_source") or "") == "targeted_negative_search_override":
    ...
    if targeted_negative_mode:
        ...
        prioritized_quote_bank = _prioritize_targeted_negative_quote_bank(
            ...
            [] if freeform_reviewer_negative_mode else targeted_negative_search_tasks,
        )
        prompt_quote_bank = _prompt_targeted_negative_quote_entries(
            prioritized_quote_bank,
            max_items=5 if freeform_reviewer_negative_mode else 3,
        )
    else:
    ...
    candidate_window_block = ""
    if targeted_negative_search_tasks and not freeform_reviewer_negative_mode:
        candidate_windows = evidence_context_meta.get("targeted_negative_candidate_windows", []) or []
        if candidate_windows:
            candidate_window_payload = {
                "candidate_relevant_paper_windows": [
                    {
                        ...
                        "negative_type": str(item.get("negative_type") or ""),
                        "required_evidence_type": str(item.get("required_evidence_type") or ""),
                        "matched_terms": _normalize_list_of_strings(item.get("matched_terms"), max_items=8, max_length=32),
                        "matched_cues": _normalize_list_of_strings(item.get("matched_cues"), max_items=4, max_length=60),
                        "window_text": _truncate_snippet_text(str(item.get("window_text") or ""), 620),
                    }
                    for item in candidate_windows[:3]
                    if isinstance(item, dict) and str(item.get("window_text") or "").strip()
                ],
                "rule": (
                    "These windows are paper-text search context for the active reviewer-negative candidate. "
                    "If one verifies the task, copy a substring from window_text as raw_quote; do not copy window labels. "
                    "If the windows show the candidate is not supported, return not_assessable rather than negative evidence."
                ),
            }
            if candidate_window_payload["candidate_relevant_paper_windows"]:
                candidate_window_block = (
                    "# Candidate-Relevant Paper Windows\n"
                    f"{json.dumps(candidate_window_payload, ensure_ascii=False, indent=2)}\n\n"
                )
    first_support_block = ""
    if first_support_needs and not targeted_negative_mode:
    ...
    if targeted_negative_search_tasks and not freeform_reviewer_negative_mode:
        targeted_negative_payload = {
            "targeted_negative_search_tasks": _prompt_targeted_negative_tasks(targeted_negative_search_tasks, max_items=2),
            "rule": (
                "Attempt the listed tasks in order, up to two. Use a copied quote/table/list/experiment description from the quote bank or excerpt. "
                "If no task-relevant copied paper text is visible, return evidence_map=[] plus one not_assessable unresolved object."
            ),
        }
        targeted_negative_block = (
            "# Targeted Negative Search Tasks\n"
            f"{json.dumps(targeted_negative_payload, ensure_ascii=False, indent=2)}\n\n"
        )
    freeform_reviewer_negative_block = ""
    if freeform_reviewer_negative_mode:
        claim_context = _prompt_freeform_reviewer_negative_claims(
            evidence_slice.get("target_claims") or state.get("claims", []),
            max_items=4,
        )
        freeform_payload = {
            "freeform_reviewer_negative_mode": "candidate_discovery_then_quote_verification_v1",
            "claim_context": claim_context,
            "reviewer_goal": (
                "Act like a peer reviewer. Identify the strongest likely weaknesses in this paper's claims. "
                "This is candidate discovery only: do not claim the issue is verified yet. Prioritize "
                "paper-internal negatives that a copied quote, result row, protocol statement, or cost "
                "tradeoff can directly verify; put missing-requirement concerns in the absence route only "
                "when you can name a concrete missing item to check against the support inventory."
            ),
            "preferred_quote_groundable_negative_types": [
                "evaluation_protocol_risk",
                "efficiency_cost_gap",
                "result_claim_mismatch",
                "negative_result",
            ...
            "absence_or_requirement_gap_types": [
                "missing_baseline",
                "unfair_or_weak_baseline",
                "missing_ablation",
                "insufficient_evaluation",
            ...
            "quote_grounding_modes": [
                "quote_groundable_internal_negative",
                "table_scope_absence",
                "absence_or_requirement_gap",
            ],
            "rule": (
                "Return up to 4 reviewer_negative_candidates, preferably covering at least 2 different real claims. Put quote_groundable_internal_negative candidates first when visible. "
                "Do not let generic missing-baseline, missing-ablation, or robustness concerns displace a protocol warning, worse result, "
                "comparison invalidation, cost tradeoff, or claim/result mismatch. Use status=pending_quote_verification only for "
                "quote_groundable_internal_negative or table_scope_absence. Use status=pending_absence_audit for absence_or_requirement_gap. "
                "For quote_groundable_internal_negative, include candidate_raw_quote copied verbatim from the quote bank or excerpt, plus quote_id "
                "and source_locator when visible. This candidate quote is a search cue only, not verified evidence. "
                "Do not return evidence_map items in this discovery stage. Each candidate must name the weakness, claim_id, negative_type, "
                "required_evidence_type, quote_grounding_mode, and a verification_question that a later Evidence Agent can use to search "
                "for copied paper text. It must also include missing_or_weak_items with concrete named baselines, datasets, components, "
                "model/settings, or evaluation conditions. For absence/coverage issues, include observed_inventory with a copied "
        ...
        freeform_reviewer_negative_block = (
            "# Freeform Reviewer Negative Search\n"
            f"{json.dumps(freeform_payload, ensure_ascii=False, indent=2)}\n\n"
        )
    open_check_payload = {
    ...
    if targeted_negative_mode:
        if freeform_reviewer_negative_mode:
            targeted_contract = (
                "# Freeform Reviewer Negative Output Contract\n"
                "First token must be <json>. Output one compact JSON object only. "
                "This is candidate discovery, not evidence verification. Return evidence_map=[] and up to 4 reviewer_negative_candidates, preferably covering at least 2 different real claims. "
                "For quote_groundable_internal_negative candidates, include candidate_raw_quote/quote_id/source_locator as search cues only. "
                "Never present a candidate as verified negative evidence.\n\n"
            )
        else:
            targeted_contract = (
                "# Targeted Negative Output Contract\n"
                "First token must be <json>. Output one compact JSON object only. "
                "When quote_grounding_mode=quote_groundable_internal_negative, return evidence only for an explicit internal protocol caveat, worse result, comparison invalidation, cost tradeoff, or direct contradiction; do not use absence reasoning for that task. "
                "If candidate_raw_quote is present, first try to re-emit that exact copied text with its quote_id/locator, but only if it truly weakens the target claim. "
                ...
                "For coverage tasks, copy concrete missing_or_weak_items into coverage_missing_items and visible evaluated items into coverage_observed_items. "
        ...
        return (
            ...
            f"{quote_bank_block}"
            f"{candidate_window_block}"
            f"# Evidence-Relevant Paper Excerpt\n{evidence_context}\n\n"
            f"{freeform_reviewer_negative_block}"
            f"{targeted_negative_block}"
            f"{targeted_contract}"
            f"# Recent Turn Log\n{_render_recent_turn_summary(task, max_items=1)}\n"
        )
    return (
...
def render_critique_observation(task: Dict[str, Any], manager_payload: Optional[Dict[str, Any]] = None) -> str:
    ...
    hard_negative_target_block = ""
    if critique_slice.get("hard_negative_diagnosis_targets"):
        hard_negative_target_block = (
            "# Hard-Negative Diagnosis Targets\n"
            f"{json.dumps({'hard_negative_diagnosis_targets': critique_slice.get('hard_negative_diagnosis_targets', [])}, ensure_ascii=False, indent=2)}\n\n"
            "# Hard-Negative Diagnosis Rules\n"
            "First use reviewer judgment over the listed claim, existing support_profile, and diagnostic_checks. "
            "A true flaw is a paper-side failure such as missing/weak baseline, missing ablation, insufficient evaluation, result-claim mismatch, method assumption gap, or reproducibility gap. "
            "Do not start from negative-sounding words. Do not treat scope/future-work/excerpt limits as grounded weaknesses. "
            "If you diagnose a plausible flaw but cannot cite verified negative evidence, output it only as status=candidate with grounding_status=diagnosis_pending_verification and omit negative_evidence_ids.\n\n"
        )
    review_issue_target_block = ""
    if critique_slice.get("review_issue_discovery_targets"):
        review_issue_target_block = (
            "# Review Issue Discovery Targets\n"
            f"{json.dumps({'review_issue_discovery_targets': critique_slice.get('review_issue_discovery_targets', [])}, ensure_ascii=False, indent=2)}\n\n"
    ...
    if negative_quote_bank:
        negative_grounding_rules_block = (
            ...
            "A quote-bank item is candidate grounding, not a verified negative evidence id; "
            "if no existing evidence has review_negative_verified, create only a candidate flaw "
            "with grounding_status=diagnosis_pending_verification and omit negative_evidence_ids. "
            "First look for true paper negatives: missing_ablation, missing_baseline, "
            "unfair_or_weak_baseline, insufficient_evaluation, missing_robustness_or_generalization, "
            "evaluation_protocol_risk, efficiency_cost_gap, result_claim_mismatch, negative_result, "
            "direct_contradiction, and concrete reproducibility_gap. Scope/future-work/limitation quotes are fallback evidence only: "
            "use them after true negatives are absent, and route them to assessment limitations unless the paper itself frames a concrete claim failure. "
    ...
    return (
        ...
        f"{hard_negative_target_block}"
        f"{review_issue_target_block}"
...
def _is_system_generated_evidence_record(item: Dict[str, Any]) -> bool:
    ...
    return (
        source in {"system recovery salvage", "fallback-extraction", "system_meta", "system-meta", "quote-bank-negative-grounding"}
...
def _is_paper_negative_evidence_record(item: Dict[str, Any]) -> bool:
    return (
        ...
        and _is_real_paper_claim_id_for_negative(item.get("claim_id"))
    )
...
def _claim_text_locatable_in_paper(
    claim_text: str,
    paper_text: str,
    *,
    ...
    """Whether a claim's content words are actually present in the paper body.

    Conservative locatability check used to gate raw-salvaged-claim
    canonicalization on the negative path: a canonicalized salvage claim may
    only host verified negative evidence if its text is locatable in the paper,
    so hallucinated / meta / truncated salvage text cannot be promoted into a
    fake reviewer-discovered negative.  Returns False when either side is empty
    so callers decide the fallback explicitly.
    """
    claim_norm = re.sub(r"\s+", " ", str(claim_text or "")).strip().lower()
    paper_norm = re.sub(r"\s+", " ", str(paper_text or "")).strip().lower()
    if not claim_norm or not paper_norm:
        return False
    claim_tokens = [
        tok
        for tok in re.findall(r"[a-z0-9]+", claim_norm)
        if len(tok) >= 4 and tok not in _SEMANTIC_STOPWORDS
    ]
    distinct = list(dict.fromkeys(claim_tokens))
    if len(distinct) < min_content_tokens:
        return False
    paper_tokens = set(re.findall(r"[a-z0-9]+", paper_norm))
    present = sum(1 for tok in distinct if tok in paper_tokens)
    return (present / len(distinct)) >= min_overlap_ratio


def _claim_obj_by_id(state: Dict[str, Any], claim_id: str) -> Optional[Dict[str, Any]]:
    target = str(claim_id or "").strip()
    if not target:
        return None
    for claim in (state or {}).get("claims", []) or []:
        if isinstance(claim, dict) and str(claim.get("claim_id") or "").strip() == target:
            return claim
    return None


def _salvage_negative_blocked_by_unlocatable_claim(item: Dict[str, Any], state: Dict[str, Any]) -> bool:
    """Block a negative whose host claim is a raw-salvaged claim not locatable in the paper.

    Only fires for canonicalized raw-salvaged claims and only when paper_text is
    available (runtime); offline recomputation over states with stripped
    paper_text keeps the prior behaviour so dashboards stay consistent.
    """
    claim_obj = _claim_obj_by_id(state, str(item.get("claim_id") or ""))
    if claim_obj is None:
        return False
    is_raw_salvage = bool(claim_obj.get("paper_claim_canonicalized_from_raw_salvage")) or (
        str(claim_obj.get("claim_origin_kind") or "").strip().lower()
        == "raw_salvaged_claim_agent_output"
    )
    if not is_raw_salvage:
        return False
    paper_text = str((state or {}).get("paper_text") or "")
    if not paper_text:
        return False
    return not _claim_text_locatable_in_paper(str(claim_obj.get("claim") or ""), paper_text)
...
def _is_grounded_paper_negative_evidence_record(item: Dict[str, Any], state: Dict[str, Any]) -> bool:
    ...
    if claim_id not in _real_claim_ids_from_state(state or {}):
        return False
    # Fix#3: a canonicalized raw-salvaged claim may only host a counted negative
    # if its text is locatable in the paper body (blocks hallucinated/meta salvage
    # from becoming a fake reviewer-discovered negative).
    if _salvage_negative_blocked_by_unlocatable_claim(item, state or {}):
        return False
    if _is_reviewer_absence_audit_evidence_record(item, state or {}):
        return True
    if item.get("runtime_evidence_verification_required") or item.get("model_claimed_verification_stripped"):
        if not _has_trusted_existing_grounding(item):
            return False
    else:
    ...
    if neg_type in {"neutral_control_context", "generic_gap", "bibliographic_or_title_noise", "neutral_instruction_noise"}:
        return False
    if not _is_review_negative_verified_evidence_record(item, state):
        return False
    return True
...
def _compact_evidence_for_prompt(items: Sequence[Dict[str, Any]], max_items: int = 6) -> List[Dict[str, Any]]:
    ...
    def score(pair: tuple[int, Dict[str, Any]]) -> tuple[int, int]:
        idx, item = pair
        if str(item.get("review_negative_label") or "") == REVIEW_NEGATIVE_VERIFIED_LABEL:
            return (0, idx)
        if item.get("strength") == "strong" and item.get("stance") in {"supports", "partially_supports"}:
            return (1, idx)
        if _is_paper_negative_evidence_record(item):
...
def _flaw_negative_grounding_conflicts(flaw: Dict[str, Any], state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for eid in explicit_ids:
        ...
        if not _is_paper_negative_evidence_record(record):
            ...
            review_label = _review_negative_label_for_record(record, state)
            ...
            conflicts.append({
                ...
                "reason": (
                    ...
                    if semantic_label == "semantic_negative_verified" and review_label and review_label != REVIEW_NEGATIVE_VERIFIED_LABEL
                    else "negative_evidence_id_not_verified"
                ),
                "verified_grounding_label": str(record.get("verified_grounding_label") or ""),
                "semantic_grounding_label": semantic_label,
                "review_negative_label": review_label,
...
def _negative_evidence_type_counts_for_flaw(flaw: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, int]:
    ...
    for eid in _flaw_valid_negative_evidence_ids(flaw, state):
        record = by_id.get(eid)
        if isinstance(record, dict):
            signature = _review_negative_dedup_signature(record, state)
...
def _explicit_negative_potential_false_positive_reason(record: Dict[str, Any]) -> str:
    quote = str(record.get("raw_quote") or record.get("evidence") or "")
    locator_text = " ".join(str(record.get(key) or "") for key in ("source", "source_locator"))
    full_text = " ".join([quote, locator_text]).strip()
    if not full_text:
        return "missing_negative_anchor_text"
    self_anchor = bool(_INTRO_NEGATIVE_SELF_ANCHOR_RE.search(quote))
    concrete_gap = bool(_REVIEW_NEGATIVE_CONCRETE_GAP_RE.search(quote))
    if _negative_anchor_is_prior_work_context(quote):
    ...
    if _REVIEW_NEGATIVE_AUTHOR_LIMITATION_RE.search(full_text):
        return "author_limitation_only"
    return ""


def _explicit_negative_record_can_surface_as_potential(record: Dict[str, Any], state: Dict[str, Any]) -> bool:
    if not isinstance(record, dict):
        return False
    if record.get("model_claimed_verification_stripped"):
        return False
    if (
        record.get("runtime_evidence_verification_required")
        and str(record.get("verified_grounding_label") or "") not in VERIFIED_PAPER_GROUNDED_LABELS
    ):
        return False
    if _is_system_generated_evidence_record(record):
        return False
    source = str(record.get("source") or "").strip().lower()
    source_locator = str(record.get("source_locator") or "").strip().lower()
    if source in _UNTRUSTED_VERIFIER_SOURCE_LABELS or source_locator in _UNTRUSTED_VERIFIER_SOURCE_LABELS:
        return False
    neg_type = _negative_evidence_type_for_record(record)
    if neg_type in {"neutral_control_context", "generic_gap", "bibliographic_or_title_noise", "neutral_instruction_noise"}:
        return False
    real_claim_ids = _real_claim_ids_from_state(state or {})
    claim_id = str(record.get("claim_id") or "").strip()
    if real_claim_ids and claim_id not in real_claim_ids:
        return False
    semantic_label = str(record.get("semantic_grounding_label") or "").strip()
    review_label = _review_negative_label_for_record(record, state)
    if review_label:
        return False
    if semantic_label not in {"", "semantic_unjudged"}:
        return False
    if _explicit_negative_potential_false_positive_reason(record):
        return False
    return bool(_normalize_text(record.get("raw_quote") or record.get("evidence"), max_length=240))
...
def _classify_flaw_final_view_layer(flaw: Dict[str, Any], state: Dict[str, Any]) -> str:
    ...
    if explicit_negative_ids:
        by_id = _evidence_records_by_id(state)
        has_unverified_review_candidate = False
...
def _negative_evidence_weakened_dimension(negative_type: str, text: str) -> str:
    ...
    if value == "missing_robustness_or_generalization":
        return "the robustness or generalization support for the claim"
    if value == "efficiency_cost_gap":
        return "the efficiency, scalability, or compute-cost support"
...
def _render_verified_negative_concern_line(
    ...
    *,
    ...
    locator = _report_safe_negative_locator(record)
    negative_type = _negative_evidence_type_for_record(record)
    readable_type = _readable_negative_evidence_type(negative_type)
    if _is_reviewer_absence_audit_evidence_record(record, state):
        bundle = record.get("review_issue_bundle") if isinstance(record.get("review_issue_bundle"), dict) else {}
        requirement = str(record.get("missing_requirement") or "").strip()
        missing_label = _REQUIREMENT_LABELS.get(requirement, readable_type)
        if _is_obligation_grounded_review_issue_evidence_record(record, state):
            parts = [f"[{flaw_status}] Verified review issue"]
        else:
            parts = [f"[{flaw_status}] Reviewer-inferred potential concern"]
        if claim_text:
            parts.append(f"targeting claim: {claim_text}")
        parts.append(f"claim-evidence gap: verified support inventory lacks {missing_label}")
        missing_or_mismatch = bundle.get("missing_or_mismatch") if isinstance(bundle, dict) else {}
        missing_items = missing_or_mismatch.get("items") if isinstance(missing_or_mismatch, dict) else []
        if missing_items:
            parts.append(f"missing/mismatch item: {', '.join(str(item) for item in missing_items[:3])}")
        inventory_count = len(bundle.get("observed_inventory", []) or []) if isinstance(bundle, dict) else 0
        if inventory_count:
            parts.append(f"observed inventory anchors: {inventory_count}")
        parts.append("grounding: claim obligation plus support-inventory audit, not a copied negative quote")
...
def _render_claim_requirement_gap_concerns(state: Dict[str, Any], max_items: int = 3) -> List[str]:
    ...
    lines: List[str] = []
    seen: set[str] = set()
    absence_flaw_claim_ids = {
        str((flaw.get("related_claim_ids") or [""])[0])
        for flaw in state.get("flaw_candidates", []) or []
        if isinstance(flaw, dict)
        and str(flaw.get("source") or "") == ABSENCE_AUDIT_SOURCE
        and _flaw_has_negative_grounding(flaw, state)
        and isinstance(flaw.get("related_claim_ids"), list)
        and flaw.get("related_claim_ids")
    }
    ...
    for item in concern_items[:max_items]:
        if str(item.get("claim_id") or "") in absence_flaw_claim_ids:
            continue
...
def _active_flaws_for_criterion(
    state: Dict[str, Any],
    pattern: str,
    *,
    review_negative_verified_only: bool = False,
) -> List[Dict[str, Any]]:
    ...
    for flaw in state.get("flaw_candidates", []) or []:
        ...
        if _is_fallback_or_meta_flaw(flaw):
            continue
        if review_negative_verified_only and not _flaw_has_negative_grounding(flaw, state):
            continue
...
def _pending_flaws_for_criterion(
    state: Dict[str, Any],
    pattern: str,
    verified_flaws: Sequence[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    verified_ids = {str(flaw.get("flaw_id") or "") for flaw in verified_flaws if isinstance(flaw, dict)}
    return [
        flaw for flaw in _active_flaws_for_criterion(state, pattern)
        if str(flaw.get("flaw_id") or "") not in verified_ids
    ]
...
def _render_criterion_assessments(state: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    *Audit Trace* section.
    ...
    novelty_pattern = r"novel|original|incremental|prior work|related work"
    significance_pattern = r"significance|impact|contribution|importance|useful|meaningful"
    soundness_pattern = r"sound|method|assumption|valid|technical|algorithm|objective|theory"
    empirical_pattern = r"experiment|evaluation|baseline|ablation|dataset|metric|table|figure|empirical|result"
    clarity_pattern = r"clarity|clear|reproduc|implementation|detail|code|hyperparameter|presentation"
    novelty_flaws = _active_flaws_for_criterion(state, novelty_pattern, review_negative_verified_only=True)
    significance_flaws = _active_flaws_for_criterion(state, significance_pattern, review_negative_verified_only=True)
    soundness_flaws = _active_flaws_for_criterion(state, soundness_pattern, review_negative_verified_only=True)
    empirical_flaws = _active_flaws_for_criterion(state, empirical_pattern, review_negative_verified_only=True)
    clarity_flaws = _active_flaws_for_criterion(state, clarity_pattern, review_negative_verified_only=True)
    pending_novelty_flaws = _pending_flaws_for_criterion(state, novelty_pattern, novelty_flaws)
    pending_significance_flaws = _pending_flaws_for_criterion(state, significance_pattern, significance_flaws)
    pending_soundness_flaws = _pending_flaws_for_criterion(state, soundness_pattern, soundness_flaws)
    pending_empirical_flaws = _pending_flaws_for_criterion(state, empirical_pattern, empirical_flaws)
    pending_clarity_flaws = _pending_flaws_for_criterion(state, clarity_pattern, clarity_flaws)
    ...
    elif pending_novelty_flaws:
        status = "mixed" if novelty_evidence else "not_assessable"
        emit("Novelty / Originality", status, "Novelty concerns are diagnosis-pending and are not treated as grounded weaknesses without review-negative evidence.", novelty_claims, novelty_evidence, pending_novelty_flaws)
    ...
    elif pending_significance_flaws:
        emit("Significance / Contribution", "not_assessable", "Contribution concerns are diagnosis-pending and need review-negative verification before they can count as paper weaknesses.", significance_claims, (), pending_significance_flaws)
    else:
    ...
    elif pending_soundness_flaws:
        status = "mixed" if method_evidence else "not_assessable"
        emit("Technical Soundness", status, "Technical concerns are diagnosis-pending; without review-negative evidence they remain follow-up questions.", method_claims, method_evidence, pending_soundness_flaws)
    ...
    elif pending_empirical_flaws:
        status = "mixed" if empirical_evidence else "not_assessable"
        emit("Empirical Adequacy", status, "Empirical concerns are diagnosis-pending; the current state has not verified them as paper-negative evidence.", empirical_claims, empirical_evidence, pending_empirical_flaws)
    ...
    elif pending_clarity_flaws:
        status = "mixed" if clarity_evidence else "not_assessable"
        emit("Clarity / Reproducibility", status, "Clarity or reproducibility concerns are diagnosis-pending until review-negative evidence verifies them.", (), clarity_evidence, pending_clarity_flaws)
...
def _classify_revision_events(revision_events: List[Dict[str, Any]], worker_payloads: Optional[List[Dict[str, Any]]] = None) -> Dict[str, List[str]]:
    ...
    for item in revision_events or []:
        ...
        elif field == 'diagnosis_pending_concern' and new_value == 'recorded':
            commit_applied = True
            commit_details.append({
                "target_id": entity,
                "old_status": "open",
                "new_status": "recorded",
                "reason": reason or "record_diagnosis_pending_concern",
                "success": True,
            })
...
def _targeted_negative_not_assessable_trace(worker_payloads: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    traces: List[Dict[str, Any]] = []
    for worker in worker_payloads or []:
        payload = (worker or {}).get("payload", {}) or {}
        trace = payload.get("targeted_negative_search_not_assessable")
        if isinstance(trace, dict) and trace:
            traces.append(trace)
    first = traces[0] if traces else {}
    return {
        "targeted_negative_not_assessable_count": len(traces),
        "targeted_negative_not_assessable_claim_id": str(first.get("target_id") or ""),
        "targeted_negative_not_assessable_task_id": str(first.get("targeted_negative_search_task_id") or ""),
        "targeted_negative_not_assessable_type": str(first.get("negative_evidence_type") or ""),
        "targeted_negative_not_assessable_required_type": str(first.get("required_evidence_type") or ""),
        "targeted_negative_not_assessable_reason": str(first.get("hygiene_status_reason") or ""),
        "targeted_negative_not_assessable_traces": copy.deepcopy(traces[:4]),
    }
...
def _compute_recovery_layer_fields(
    ...
    hygiene_delta_improved = bool(
        ...
        and (
            ...
            or (operation == "mark_contested" and contested_relation_added)
        )
    )
    no_effect_commit = bool(
        ...
        and not diagnosis_pending_concern_added
        and not negative_recovery_commit
    )

    if hygiene_delta_improved:
        layer = "hygiene_delta_improved"
    elif diagnosis_pending_recorded:
        layer = "diagnosis_pending_recorded"
    ...
    return {
        ...
        "recovery_layer_diagnosis_pending_recorded": diagnosis_pending_recorded,
...
def build_turn_log(
    ...
    targeted_not_assessable_trace = _targeted_negative_not_assessable_trace(worker_payloads)
    targeted_active_tasks = [
        item for item in manager_payload.get("targeted_negative_search_active_tasks", []) or []
        if isinstance(item, dict)
    ]
    targeted_active_task_ids = list(dict.fromkeys(
        [
            str(item.get("task_id") or "")
            for item in targeted_active_tasks[:3]
            if str(item.get("task_id") or "")
        ]
        + [
            str(item or "")
            for item in manager_payload.get("targeted_negative_search_active_task_ids", []) or []
            if str(item or "")
        ]
    ))
    targeted_active_candidate_ids = list(dict.fromkeys(
        [
            str(item.get("reviewer_negative_candidate_id") or "")
            for item in targeted_active_tasks[:3]
            if str(item.get("reviewer_negative_candidate_id") or "")
        ]
        + [
            str(item or "")
            for item in manager_payload.get("targeted_negative_search_active_candidate_ids", []) or []
            if str(item or "")
        ]
    ))
    return {
        ...
        "hard_negative_diagnosis_required": bool(manager_payload.get("hard_negative_diagnosis_required", False)),
        "freeform_reviewer_negative_enabled": bool(manager_payload.get("freeform_reviewer_negative_enabled", False)),
        "freeform_reviewer_negative_mode": manager_payload.get("freeform_reviewer_negative_mode", ""),
        "freeform_reviewer_negative_stage": manager_payload.get("freeform_reviewer_negative_stage", ""),
        "freeform_reviewer_negative_discovery_required": bool(
            manager_payload.get("freeform_reviewer_negative_discovery_required", False)
        ),
        "reviewer_negative_candidate_count": len(state.get("reviewer_negative_candidates", []) or []),
        "targeted_negative_search_required": bool(manager_payload.get("targeted_negative_search_required", False)),
        "targeted_negative_search_task_count": manager_payload.get("targeted_negative_search_task_count", 0),
        "targeted_negative_search_active_tasks": copy.deepcopy(targeted_active_tasks[:3]),
        "targeted_negative_search_active_task_ids": targeted_active_task_ids[:3],
        "targeted_negative_search_active_candidate_ids": targeted_active_candidate_ids[:3],
        "targeted_negative_candidate_window_count": _normalize_int(
            manager_payload.get("targeted_negative_candidate_window_count", 0),
            default=0,
            min_value=0,
        ),
        "targeted_negative_candidate_window_ids": [
            str(item.get("window_id") or "")
            for item in manager_payload.get("targeted_negative_candidate_windows", [])[:3]
            if isinstance(item, dict) and str(item.get("window_id") or "")
        ],
        "targeted_negative_search_no_task_blocked": bool(manager_payload.get("targeted_negative_search_no_task_blocked", False)),
        **targeted_not_assessable_trace,
        "negative_evidence_formation_required": bool(
            not manager_payload.get("hard_negative_diagnosis_required", False)
            and (
                ...
                or (
                    ...
                    and manager_payload.get("targeted_negative_search_required", False)
                )
            )
        ),
        "compact_negative_pass_required": bool(
            manager_payload.get("compact_negative_pass_required", False)
            or manager_payload.get("policy_source") == "compact_negative_pass_override"
        ),
        "negative_pass_mode": manager_payload.get(
            "negative_pass_mode",
            "compact" if manager_payload.get("policy_source") == "compact_negative_pass_override" else "",
        ),
        ...
        "evidence_json_raw_head": str(manager_payload.get("evidence_json_raw_head", ""))[:360],
        "evidence_json_raw_tail": str(manager_payload.get("evidence_json_raw_tail", ""))[:360],
        ...
        "missing_requirements": turn_patch_log.get("missing_requirements", []),
        "missing_negative_types": turn_patch_log.get("missing_negative_types", []),
        ...
        "diagnosis_pending_concern_claim_id": turn_patch_log.get("diagnosis_pending_concern_claim_id", ""),
        "diagnosis_pending_concern_missing_requirements": turn_patch_log.get("diagnosis_pending_concern_missing_requirements", []),
        "diagnosis_pending_concern_missing_negative_types": turn_patch_log.get("diagnosis_pending_concern_missing_negative_types", []),
        "diagnosis_pending_concern_final_view": turn_patch_log.get("diagnosis_pending_concern_final_view", ""),
        "diagnosis_pending_concern_status": turn_patch_log.get("diagnosis_pending_concern_status", ""),
        "sticky_target_id": state.get("sticky_target_id", ""),
        "sticky_target_type": state.get("sticky_target_type", ""),
