...
基于 clean 4B fulltest39 的离线审计结果，冻结一版更安全的 final-view 推荐口径。该口径不改变 runtime、不修改 live `ReviewState`，只用于论文结果层和 case 分析。
...
1. 使用 `MAINLINE_FINAL_V1_CLEAN_4B_LIFECYCLE_AUDIT_V1.json` 作为唯一输入。
