...
DEFAULT_SUBSET_PATH = "outputs/review_infer/p24_3_recovery_emission_subset.parquet"
DEFAULT_META_PATH = "outputs/review_infer/p24_3_recovery_emission_subset_meta.json"
DEFAULT_ANALYSIS_PATH = "outputs/review_infer/p24_3_recovery_emission_selection.json"
DEFAULT_OUTPUT_PATH_4B = "outputs/review_infer/p24_3_4b_regression.jsonl"
...
def select_recovery_emission_cases(
    aggregated: Dict[str, Dict[str, Any]],
    max_samples: int,
) -> List[Dict[str, Any]]:
    candidates = [
        metric for metric in aggregated.values()
        if metric["recovery_triggered_count"] > 0
        and (
            metric["patch_committed_count"] > 0
    ...
    for bucket_name in ["committed", "emitted_no_commit", "triggered_no_patch"]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Construct and optionally run the p24.3 recovery-emission subset.")
    ...
    recovery_emission_cases = select_recovery_emission_cases(aggregated, max_samples=args.max_emission_samples)
    sentinel_info = collect_historical_sentinels(aggregated, sentinel_ids)
    materialized = materialize_subset(dataset_paths, [item["paper_id"] for item in recovery_emission_cases], Path(args.subset_path))
    ...
    meta = {
        ...
        "recovery_emission_cases": recovery_emission_cases,
        **sentinel_info,
        **materialized,
    }
    analysis = {
    ...
    if materialized["materialized_count"] == 0:
        print("No rows materialized for recovery-emission subset; aborting run.")
        return 1
    ...
    print("Running 4B recovery-emission regression:")
    print(" ".join(cmd_4b))
