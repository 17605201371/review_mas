...
launch_run() {
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    cat <<'EOF'
+ DRMAS_NEG_QUOTE_HYGIENE=1 DRMAS_TARGETED_NEGATIVE_SEARCH=1 DRMAS_FREEFORM_REVIEWER_NEGATIVE=1 DRMAS_REVIEW_ISSUE_BUNDLE=1 API_MAX_WORKERS=4 API_MAX_RETRIES=8 API_TIMEOUT=600 MAX_TOKENS=2048 bash run_hardneg20_guard3.sh
EOF
    return 0
  fi
