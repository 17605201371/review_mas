...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        task_pass_lst = []
        reward_extra_info_lists = defaultdict(list)
        # success_rate_dict = {}
        ...
        for test_data in self.val_dataloader:
            ...
            for key, values in reward_extra_info.items():
                reward_extra_info_lists[key].extend(values)
        ...
        for metric_name, metric_values in reward_extra_info_lists.items():
            if len(metric_values) != reward_tensor.shape[0]:
                continue
            for data_source in set(data_sources):
                ds_values = [metric_values[i] for i, ds in enumerate(data_sources) if ds == data_source]
                if ds_values:
                    metric_dict[f'val/{data_source}/{metric_name}'] = float(np.mean(ds_values))
